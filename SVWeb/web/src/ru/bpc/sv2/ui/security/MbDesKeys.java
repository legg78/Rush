...
@ViewScoped
@ManagedBean (name = "MbDesKeys")
public class MbDesKeys extends AbstractBean {
	private static final long serialVersionUID = 1L;

	private static final Logger logger = Logger.getLogger("SECURITY");

	private static String COMPONENT_ID = "desKeysTable";
	...
	public List<SelectItem> getNameFormats() {
		return getDictUtils().getLov(LovConstants.PRINT_FORMATS);
	}

	public void changePrintComponents(ValueChangeEvent event) {
		Boolean enabled = (Boolean) event.getNewValue();
		newDesKey.setPrintComponents(enabled.booleanValue());
	}
	...
	public List<SelectItem> getKeyPrefixes() {
		List<SelectItem> result = null;
		if (curMode == VIEW_MODE) {
			result = getDictUtils().getArticles(DictNames.ENCRYPTION_KEY_PREFIX, true);
		} else {
		...
		try {
			...
			if (filterRec != null) {
				...
				if (filterRec.get("entityType") != null) {
					desKeyFilter.setEntityType(filterRec.get("entityType"));
				}
				if (filterRec.get("keyValue") != null) {
					desKeyFilter.setKeyValue(filterRec.get("keyValue"));
				}
				if (filterRec.get("keyLength") != null) {
					desKeyFilter.setKeyLength(filterRec.get("keyLength"));
				}
				if (filterRec.get("keyPrefix") != null) {
					desKeyFilter.setKeyPrefix(filterRec.get("keyPrefix"));
				}
				if (filterRec.get("keyType") != null) {
					desKeyFilter.setKeyType(filterRec.get("keyType"));
				}
				if (filterRec.get("lmkId") != null) {
					desKeyFilter.setLmkId(Integer.parseInt(filterRec.get("lmkId")));
				}
			}
	...
	public void saveSectionFilter() {
		try {
			...
			desKeyFilter = getFilter();
			if (desKeyFilter.getEntityType() != null) {
				filterRec.put("entityType", desKeyFilter.getEntityType());
			}
			if (desKeyFilter.getKeyValue() != null) {
				filterRec.put("keyValue", desKeyFilter.getKeyValue());
			}
			if (desKeyFilter.getKeyLength() != null) {
				filterRec.put("keyLength", desKeyFilter.getKeyLength());
			}
			if (desKeyFilter.getKeyPrefix() != null) {
				filterRec.put("keyPrefix", desKeyFilter.getKeyPrefix());
			}
			if (desKeyFilter.getKeyType() != null) {
				filterRec.put("keyType", desKeyFilter.getKeyType());
			}
			if (desKeyFilter.getLmkId() != null) {
				filterRec.put("lmkId", desKeyFilter.getLmkId().toString());
			}
