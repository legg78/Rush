create or replace package body com_prc_dict_export_pkg is
...
begin
    trc_log_pkg.debug(
        i_text       => 'com_prc_dict_export_pkg.process: START with i_dict_version [#1], i_array_dictionary_id [#2], i_lang [#3], i_inst_id [#4]'
    ...
    trc_log_pkg.debug('com_prc_dict_export_pkg.process: FINISH');

exception
    when others then
        rollback to sp_mpt_dict_export;
        prc_api_stat_pkg.log_end(
