create or replace force view hsm_ui_device_parameter_vw as
    with ver
         as (select i.id object_id
                  , 'ENTTHSMD' entity_type
                  , v.version_id
                  , p.standard_id
                  , p.param_id
                  , p.data_type
              ...
              where i.id = v.object_id
                and v.entity_type = 'ENTTHSMD'
                and v.version_id = p.version_id
                and p.param_entity_type = 'ENTTHSMD')
       , std
         as (select i.id object_id
                  , 'ENTTHSMD' entity_type
                  , null version_id
              ...
              where i.id = s.object_id
                and s.entity_type = 'ENTTHSMD'
                and s.standard_id = p.standard_id
                and p.entity_type = 'ENTTHSMD')
    select 'ENTTSTVR' param_level
         ...
         , get_number_value(
               ver.data_type
             , nvl(def_val.param_value, ver.default_value)
           )
         ...
         , get_lov_value(
             ...
             , ver.lov_id
               ...
               default_lov_value
         , get_number_value(ver.data_type, val.param_value)
               param_number_value
         ...
         , get_lov_value(ver.data_type, val.param_value, ver.lov_id)
               param_lov_value
         , val.mod_id
         , get_text('rul_mod', 'name', val.mod_id, ver.lang) as mod_name
         , ver.scale_id
         , ver.scale_name
         , def_val.xml_value default_xml_value
         , val.xml_value
      from ver, cmn_parameter_value val, cmn_parameter_value def_val
    ...
    select 'ENTTSTDR' param_level
         ...
         , std.param_name
         , std.lang
         , get_number_value(std.data_type, std.default_value)
               default_number_value
         , get_char_value(std.data_type, std.default_value)
               default_char_value
         , get_date_value(std.data_type, std.default_value)
               default_date_value
         , get_lov_value(std.data_type, std.default_value, std.lov_id)
               default_lov_value
         , get_number_value(std.data_type, val.param_value)
               param_number_value
         ...
         , get_lov_value(std.data_type, val.param_value, std.lov_id)
               param_lov_value
         , val.mod_id
         ...
         , val.xml_value
      from std, cmn_parameter_value val
