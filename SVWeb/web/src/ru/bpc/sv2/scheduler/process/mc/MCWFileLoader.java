...
public class MCWFileLoader implements FileSaver {
	...
	public void save() throws Exception {
		...
		if ((fileObject != null) && (con != null) && (fileAttrs != null)) {
			try {
				 ...
				 * RDW without blocks
				 */
				// TODO: if someone needs this then it's necessary to add batch
				// data retrieving like it's done below
//				while (rs.next()) {
...
//				}
//
//				out.write(DatatypeConverter.parseHexBinary(line));
				...
				while (!finish) {
					while (line.length() < (BLOCK_SIZE - 2) * 2) { // because every symbol is a half byte
						...
						if (!rs.next()) {
							...
							rs = getData(pstmt, i);
							if (!rs.next()) { // EOF
								// this can lead to line.length() > BLOCK_SIZE - 2 (see first IF inside this loop)
								line += eof();
								str = null;
								break;
							}
...
//		String sourceString = new String(DatatypeConverter.parseHexBinary(hexStr), sourceEnc);
//		byte[] destBytes = sourceString.getBytes(destEnc);
