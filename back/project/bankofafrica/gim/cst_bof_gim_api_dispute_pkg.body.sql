create or replace package body cst_bof_gim_api_dispute_pkg is
...
begin
    ...
    cst_bof_gim_api_fin_msg_pkg.get_fin_message(
    ...
    -- Checks
    if i_select_item in (cst_bof_gim_api_const_pkg.DSP_ITEM_RVRSL_ON_FIRST_PRES
                       , cst_bof_gim_api_const_pkg.DSP_ITEM_RVRSL_ON_SECOND_PRES
                       , cst_bof_gim_api_const_pkg.DSP_ITEM_RVRSL_ON_PRES_CHRGBCK)
    then
        l_trans_code_1 :=
            case i_select_item
                when cst_bof_gim_api_const_pkg.DSP_ITEM_RVRSL_ON_PRES_CHRGBCK
                then '3'
                else '2'
            end;
    ...
    l_fin_rec.is_reversal :=
        case i_select_item
            when cst_bof_gim_api_const_pkg.DSP_ITEM_RVRSL_ON_FIRST_PRES   then com_api_const_pkg.TRUE
            when cst_bof_gim_api_const_pkg.DSP_ITEM_RVRSL_ON_SECOND_PRES  then com_api_const_pkg.TRUE
            when cst_bof_gim_api_const_pkg.DSP_ITEM_RVRSL_ON_PRES_CHRGBCK then com_api_const_pkg.TRUE
                                                                          else com_api_const_pkg.FALSE
        end;

    l_fin_rec.trans_code :=
        case i_select_item
            when cst_bof_gim_api_const_pkg.DSP_ITEM_RVRSL_ON_FIRST_PRES    -- 1 - Reversal on First Presentment
                then '2' || substr(l_fin_rec.trans_code, 2)    -- TC_*_REVERSAL
            when cst_bof_gim_api_const_pkg.DSP_ITEM_FIRST_CHARGEBACK       -- 2 - Chargeback on TC05, TC06, TC07
                then '1' || substr(l_fin_rec.trans_code, 2)    -- TC_*_CHARGEBACK
            when cst_bof_gim_api_const_pkg.DSP_ITEM_SECOND_PRESENTMENT     -- 3 - Second Presentment on TC05, TC06, TC07
                then '0' || substr(l_fin_rec.trans_code, 2)    -- TC_*
            when cst_bof_gim_api_const_pkg.DSP_ITEM_RVRSL_ON_SECOND_PRES   -- 4 - Reversal on Second Presentment
                then '2' || substr(l_fin_rec.trans_code, 2)    -- TC_*_REVERSAL
            when cst_bof_gim_api_const_pkg.DSP_ITEM_RVRSL_ON_PRES_CHRGBCK  -- 5 - Presentment Chargeback Reversal
                then '3' || substr(l_fin_rec.trans_code, 2)    -- TC_*_CHARGEBACK_REV
            when cst_bof_gim_api_const_pkg.DSP_ITEM_SECOND_PRES_CHRGBCK    -- 6 - Chargeback on Second Presentment
                then '1' || substr(l_fin_rec.trans_code, 2)    -- TC_*_CHARGEBACK
            else
                l_fin_rec.trans_code
        end;

    l_fin_rec.spec_chargeback_ind := cst_bof_gim_api_fin_msg_pkg.get_spec_chargeback_ind(
                                         i_trans_code        => l_fin_rec.trans_code
                                       , i_chargeback_amount => nvl(i_oper_amount, 0)
                                       , i_original_amount   => nvl(l_fin_rec.oper_amount, 0)
                                     );

    if i_select_item in (cst_bof_gim_api_const_pkg.DSP_ITEM_RVRSL_ON_PRES_CHRGBCK) then
    ...
    l_fin_rec.usage_code :=
        case
            when i_select_item in (cst_bof_gim_api_const_pkg.DSP_ITEM_SECOND_PRESENTMENT
                                 , cst_bof_gim_api_const_pkg.DSP_ITEM_RVRSL_ON_SECOND_PRES)
                then '2'
            when i_select_item in (cst_bof_gim_api_const_pkg.DSP_ITEM_RVRSL_ON_FIRST_PRES)
                then '1'
            when i_select_item in (cst_bof_gim_api_const_pkg.DSP_ITEM_RVRSL_ON_PRES_CHRGBCK)
    ...
    l_fin_rec.reason_code :=
        case
            when i_select_item in (cst_bof_gim_api_const_pkg.DSP_ITEM_FIRST_CHARGEBACK
                                 , cst_bof_gim_api_const_pkg.DSP_ITEM_SECOND_PRES_CHRGBCK)
            then i_reason_code
    ...
    l_fin_rec.chargeback_ref_num :=
        case
            when i_select_item in (cst_bof_gim_api_const_pkg.DSP_ITEM_RVRSL_ON_PRES_CHRGBCK
                                 , cst_bof_gim_api_const_pkg.DSP_ITEM_SECOND_PRESENTMENT)
                then l_fin_rec.chargeback_ref_num
            when i_select_item in (cst_bof_gim_api_const_pkg.DSP_ITEM_RVRSL_ON_FIRST_PRES)
             and l_dispute_id is null
                then lpad('0', 6, '0')
            when i_select_item in (cst_bof_gim_api_const_pkg.DSP_ITEM_FIRST_CHARGEBACK)
                then lpad(nvl(to_char(mod(nvl(l_fin_rec.dispute_id, l_dispute_id), 1000000)), '0'), 6, '0')
            when i_select_item not in (cst_bof_gim_api_const_pkg.DSP_ITEM_RVRSL_ON_FIRST_PRES)
             and l_dispute_id is null
    ...
    l_stage  := 'put_message';
    o_fin_id := cst_bof_gim_api_fin_msg_pkg.put_message(i_fin_rec => l_fin_rec);
...
procedure generate_retrieval_request(
  ...
  , i_response_type                 in     com_api_type_pkg.t_byte_char
    ...
    l_original_fin_rec              cst_bof_gim_api_type_pkg.t_gim_fin_mes_rec;
    l_fin_rec                       cst_bof_gim_api_type_pkg.t_gim_fin_mes_rec;
...
begin
    ...
    cst_bof_gim_api_fin_msg_pkg.put_retrieval(i_retrieval_rec => l_retrieval_rec);
...
begin
    ...
    l_stage := 'network_id and inst_id';
    ...
    l_stage := 'put_fee';
    cst_bof_gim_api_fin_msg_pkg.put_fee(i_fee_rec => l_fee_rec);
...
begin
    ...
    l_fraud_rec.logical_file             := cst_bof_gim_api_const_pkg.TC_FL_HEADER;
    ...
    l_stage := 'put_fraud';
    cst_bof_gim_api_fin_msg_pkg.put_fraud(i_fraud_rec => l_fraud_rec);
    ...
    l_fin_rec.file_id     := null;
    l_fin_rec.status      := net_api_const_pkg.CLEARING_MSG_STATUS_READY;
    l_fin_rec.trans_code  := cst_bof_gim_api_const_pkg.TC_FRAUD_ADVICE;
