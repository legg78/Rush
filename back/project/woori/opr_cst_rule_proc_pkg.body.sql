...
begin
    ...
    select cdb.repay_priority
     ...
     where cd.id = cdb.debt_id
       ...
       and cd.macros_type_id = cst_woo_const_pkg.MACROS_TYPE_ID_DEBIT_ON_OPER --1004
       and cdb.balance_type = cst_woo_const_pkg.BALANCE_TYPE_OVERDRAFT --'BLTP1002'
       ;
    ...
    select cdb.id
     ...
     where cd.id = cdb.debt_id
       and cd.split_hash = cdb.split_hash
       and cd.oper_id = l_oper_id_orig
       and cd.macros_type_id = cst_woo_const_pkg.MACROS_TYPE_ID_DEBIT_FEE --1007
       and cdb.balance_type = cst_woo_const_pkg.BALANCE_TYPE_OVERDRAFT --'BLTP1002'
       ;

    opr_api_shared_data_pkg.set_param(
        i_name      => 'FEE_REPAY_PRIOR'
      , i_value     => l_fee_repay_prior
    );

    update crd_debt_balance
       set repay_priority = l_orig_repay_prior + 1
...
begin
    ...
    l_fee_repay_prior := opr_api_shared_data_pkg.get_param_num(
        i_name              => 'FEE_REPAY_PRIOR'
      , i_mask_error        => com_api_type_pkg.TRUE
    );
    ...
    update crd_debt_balance
       set repay_priority = l_fee_repay_prior
     where id = l_debt_balance_id;
