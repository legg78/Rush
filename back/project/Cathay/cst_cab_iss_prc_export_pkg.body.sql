CREATE OR REPLACE package body cst_cab_iss_prc_export_pkg as
/************************************************************
 * API custom for Cathay process files <br />
 * Created by ChauHuynh (huynh@bpcbt.com)  at 13.03.2019 <br />
 * Module: cst_cab_iss_prc_export_pkg <br />
 * @headcom
 ***********************************************************/
    ...
    if l_service_id is not null then
        ...
        for i in 1 .. l_limit_type_tab.count loop
            ...
            o_xml_block := o_xml_block
                        ...
                        || '<next_date>'    || l_next_date                      || '</next_date>'
    ...
    if l_is_notification = com_api_type_pkg.TRUE then
        ...
        select case
         ...
         where n.entity_type       = iss_api_const_pkg.ENTITY_TYPE_CARDHOLDER
           ...
           and (n.start_date      is null or n.start_date  < i_sysdate)
           and (n.end_date        is null or n.end_date    > i_sysdate)
    ...
    select xmlagg(
      ...
      from (
              select a.account_number
                   ...
                   , ao.is_atm_currency
                   , ao.is_pos_currency
                   , row_number() over (partition by ao.object_id, a.account_type order by a.id) account_seq_number
                   , 1 as link_flag
  ...
  , i_replace_inst_id_by_number  in     com_api_type_pkg.t_boolean    default com_api_const_pkg.FALSE
        ...
        select crd.inst_id           as inst_id
             ...
             , xmlconcat(
                    xmlforest(
                      ...
                      , case
                            when pm.pin_verify_method = prs_api_const_pkg.PIN_VERIFIC_METHOD_PVV
                            then cd.pvv
                        end                           as "pvv"
                        -- DF8077 in block FF41 of CREF
                      , case
                            when pm.pin_verify_method in (prs_api_const_pkg.PIN_VERIFIC_METHOD_COMBINED
                                                        , prs_api_const_pkg.PIN_VERIFIC_METHOD_IBM_3624)
                            then cd.pin_offset
                        end                           as "pin_offset"
                        -- DF8163
                      , case
            ...
            open o_cursor for
                select /*+ ordered use_nl(sm, eo, ci) full(sm) index(eo evt_event_object_status) index(ci iss_card_instance_uk) */
                 ...
                 where l_thread_number in (sm.thread_number, prc_api_const_pkg.DEFAULT_THREAD)
                   ...
                   and eo.entity_type            = iss_api_const_pkg.ENTITY_TYPE_CARD
                   and (eo.inst_id               = i_inst_id       or i_inst_id       is null)
    ...
    begin
        if l_instance_id_tab.count() > 0 then
            ...
            /*dbms_lob.copy(dest_lob    => l_file /*l_cur_clob* /,
                          src_lob     => l_tmp_clob,
                          amount      => dbms_lob.getlength(l_tmp_clob),
                          dest_offset => 1,
                          src_offset  => 1);
          
            if dbms_lob.isopen(l_file /*l_cur_clob* /) = 1 then
              dbms_lob.close(l_file /*l_cur_clob* /);
            end if;*/
    ...
    loop
        begin
            ...
            if l_full_export = com_api_type_pkg.TRUE then
                ...
                trc_log_pkg.debug('l_instance_id_tab.count = ' || l_instance_id_tab.count);
...
end cst_cab_iss_prc_export_pkg;
/
