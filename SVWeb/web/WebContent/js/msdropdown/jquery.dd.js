...
msBeautify = {
	...
	debug: function (v) {
		if (v !== false) {
			query(".ddOutOfVision").css({height: 'auto', position: 'relative'});
		} else {
			query(".ddOutOfVision").css({height: '0px', position: 'absolute'});
		}
	},
	oldDiv: '',
...
function dd(element, settings) {
	...
	var query = function(ele) {
		var pos = ele.indexOf(":");
		if (pos != -1) {
			ele = ele.substring(0, pos) + "\\" + ele.substring(pos);
		}
		return $(ele);
	}; 	
	var _getIndex = function(opt) {
		var childid = _getPostID("postChildID"); 
		return query("#"+childid + " li."+_styles_i.li).index(opt);
	};
	...
	var _construct = function() {		
		...
		var hasMainCSS = query("#"+_element).data("maincss");
		if(hasMainCSS){
		...
		_updateProp("selectedOptions", query("#"+_element +" option:selected"));
	 };	
	 /********************************************************************************************/	
	...
	var _hideOriginal = function() {
		...
		if (query("#"+hidid).length==0) {			 
			...
			query("#"+_element).after(oDiv);
			query("#"+_element).appendTo(query("#"+hidid));
		} else {
			query("#"+hidid).css({height: 0,overflow: 'hidden',position: 'absolute'});
		};
	};
	var _createWrapper = function () {
		...
		var w = query("#" + _element).outerWidth();
	...
	var _createTitle = function () {
		...
		//check sprite
		var useSprite = query("#"+_element).data("usesprite");
	...
	var _childHeight = function (val) {
		...
		if (val) {
			if (val == -1) { //auto
				query("#"+childid).css({height: "auto", overflow: "auto"});
			} else {				
				query("#"+childid).css("height", val+"px");
			};
			return false;
		};
		//else return height
		...
		if (getElement(_element).options.length > _settings.visibleRows) {
			var margin = parseInt(query("#" + childid + " li:first").css("padding-bottom")) + parseInt(query("#" + childid + " li:first").css("padding-top"));
			if(_settings.rowHeight===0) {
				query("#" + childid).css({visibility:'hidden',display:'block'}); //hack for first child
				_settings.rowHeight = Math.round(query("#" + childid + " li:first").height());
				if (_settings.rowHeight == 0){					
					_settings.rowHeight = _settings.rowFixHeight;
				}
				query("#" + childid).css({visibility:'visible'});
				if(!_isList) {
					query("#" + childid).css({display:'none'});				
				};
			};
			iHeight = ((_settings.rowHeight + margin) * _settings.visibleRows);			
			
		} else if (_isList) {
			iHeight = query("#" + _element).height(); //get height from original element			
		};		
		return iHeight;
	};
	var _applyChildEvents = function () {
		var childid = _getPostID("postChildID");
		query("#" + childid).on("click", function (e) {
		...
		query("#" + childid + " li." + _styles.enabled).on("click", function (e) {
			_close(this);
		});
		query("#" + childid + " li." + _styles.enabled).on("mousedown", function (e) {
			if (_isDisabled == true) return false;
			_oldSelected = query("#" + childid + " li." + _styles.selected);
			_lastTarget = this;
			e.preventDefault();
			e.stopPropagation();
			if (_isList === true) {
				if (_isMultiple) {
					if (_shiftHolded === true) {
						...
						var selected = query("#" + childid + " li." + _styles.selected);
						var lastIndex = _getIndex(this);
						if (selected.length > 1) {
							var items = query("#" + childid + " li." + _styles_i.li);
						...
						query("#" + childid + " li." + _styles.selected).removeClass(_styles.selected);
						$(this).addClass(_styles.selected);
					};
				} else {
					query("#" + childid + " li." + _styles.selected).removeClass(_styles.selected);
		...
		query("#" + childid + " li." + _styles.enabled).on("mouseenter", function (e) {
			if (_isDisabled == true) return false;
			e.preventDefault();
			e.stopPropagation();
			if (_lastTarget != null) {
		...
		query("#" + childid + " li." + _styles.enabled).on("mouseover", function (e) {
			if (_isDisabled == true) return false;
			$(this).addClass(_styles.hover);
		});
		query("#" + childid + " li." + _styles.enabled).on("mouseout", function (e) {
			if (_isDisabled == true) return false;
			query("#" + childid + " li." + _styles.hover).removeClass(_styles.hover);
		});
	
		query("#" + childid + " li." + _styles.enabled).on("mouseup", function (e) {
			if (_isDisabled == true) return false;
			e.preventDefault();
			e.stopPropagation();
			var selected = query("#" + childid + " li." + _styles.selected).length;
		...
		if (_settings.disabledOptionEvents == false) {
			query("#" + childid + " li." + _styles_i.li).on("click", function (e) {
				if (_isDisabled == true) return false;
				fireOptionEventIfExist(this, "click");
			});
			query("#" + childid + " li." + _styles_i.li).on("mouseenter", function (e) {
				if (_isDisabled == true) return false;
				fireOptionEventIfExist(this, "mouseenter");
			});
			query("#" + childid + " li." + _styles_i.li).on("mouseover", function (e) {
				if (_isDisabled == true) return false;
				fireOptionEventIfExist(this, "mouseover");
			});
			query("#" + childid + " li." + _styles_i.li).on("mouseout", function (e) {
				if (_isDisabled == true) return false;
				fireOptionEventIfExist(this, "mouseout");
			});
			query("#" + childid + " li." + _styles_i.li).on("mousedown", function (e) {
				if (_isDisabled == true) return false;
				fireOptionEventIfExist(this, "mousedown");
			});
			query("#" + childid + " li." + _styles_i.li).on("mouseup", function (e) {
				if (_isDisabled == true) return false;
	...
	var _removeChildEvents = function () {
		var childid = _getPostID("postChildID");
		query("#" + childid).off("click");
		query("#" + childid + " li." + _styles.enabled).off("mouseenter");
		query("#" + childid + " li." + _styles.enabled).off("click");
		query("#" + childid + " li." + _styles.enabled).off("mouseover");
		query("#" + childid + " li." + _styles.enabled).off("mouseout");
		query("#" + childid + " li." + _styles.enabled).off("mousedown");
		query("#" + childid + " li." + _styles.enabled).off("mouseup");
	};
	var _applyEvents = function () {
		var id = _getPostID("postID");
		var childid = _getPostID("postChildID");		
		query("#" + id).on(_settings.event, function (e) {			
			if (_isDisabled == true) return false;
		...
		_applyChildEvents();		
		query("#" + id).on("dblclick", on_dblclick);
		query("#" + id).on("mousemove", on_mousemove);
		query("#" + id).on("mouseenter", on_mouseover);
		query("#" + id).on("mouseleave", on_mouseout);
		query("#" + id).on("mousedown", on_mousedown);
		query("#" + id).on("mouseup", on_mouseup);
	};
	//after create
	var _fixedForList = function () {
		...
		var childid = _getPostID("postChildID");		
		if (_isList === true) {
			query("#" + id + " ." + _styles.ddTitle).hide();
			query("#" + childid).css({display: 'block', position: 'relative'});	
			//_open();
		} else {
			_isMultiple = false; //set multiple off if this is not a list
			query("#" + id + " ." + _styles.ddTitle).show();
			query("#" + childid).css({display: 'none', position: 'absolute'});
			//set value
			var first = query("#" + childid + " li." + _styles.selected)[0];
			query("#" + childid + " li." + _styles.selected).removeClass(_styles.selected);
	...
	var _fixedForDisabled = function () {
		...
		if (_isDisabled === true) {
			query("#" + id).addClass(_styles.disabledAll);
		} else {
			query("#" + id).removeClass(_styles.disabledAll);
		};
	};
	var _fixedSomeUI = function () {
		//auto filter
		var tid = _getPostID("postTitleTextID");
		query("#" + tid).on("keyup", _applyFilters);
		//if is list
		_fixedForList();
		_fixedForDisabled();
	};
	...
	var _selectMutipleOptions = function (bySelected) {
		var childid = _getPostID("postChildID");
		var selected = bySelected || query("#" + childid + " li." + _styles.selected);
	...
	var _fireAfterItemClicked = function () {
		...
		if (selected.length == 0) {
			...
			var index = query("#" + childid + " li." + _styles.selected);
		} else {
			//if one selected
			var index = _getIndex(query("#" + childid + " li." + _styles.selected));
		};
		if (getElement(_element).selectedIndex != index || _forcedTrigger) {
			_setValue(index);
			query("#" + _element).trigger("change");
		};
	};
	...
	var _applyFilters = function () {
		...
		if (sText.length == 0) {
			query("#" + childid + " li:hidden").show(); //show if hidden
			_childHeight(_childHeight());
		} else {
			query("#" + childid + " li").hide();
			query("#" + childid + " li:Contains('" + sText + "')").show();	
			if (query("#" + childid + " li:visible").length <= _settings.visibleRows) {					
	...
	var _showFilterBox = function () {
		var tid = _getPostID("postTitleTextID");
		if (query("#" + tid + ":hidden").length > 0 && _controlHolded == false) {
			query("#" + tid + ":hidden").show().val("");
			getElement(tid).focus();
		};
	};
	var _hideFilterBox = function () {
		var tid = _getPostID("postTitleTextID");
		if (query("#" + tid + ":visible").length > 0) {
			query("#" + tid + ":visible").hide();
			getElement(tid).blur();
		};
	};
	...
	var _scrollToIfNeeded = function (opt) {
		var childid = _getPostID("postChildID");
		//if scroll is needed
		opt = (opt !== undefined) ? opt : query("#" + childid + " li." + _styles.selected);
		if (opt.length > 0) {
			var pos = parseInt(($(opt).position().top));
			var ch = parseInt(query("#" + childid).height());
			if (pos > ch) {
				var top = pos + query("#" + childid).scrollTop() - (ch/2);
				query("#" + childid).animate({scrollTop:top}, 500);
			};
		};
	};
	var _next = function () {
		var childid = _getPostID("postChildID");
		var items = query("#" + childid + " li:visible." + _styles_i.li);
		var selected = query("#" + childid + " li:visible." + _styles.selected);
		selected = (selected.length==0) ? items[0] : selected;
		var index = query("#" + childid + " li:visible." + _styles_i.li).index(selected);
		if ((index < items.length - 1)) {
			index = getNext(index);
			if (index < items.length) { //check again - hack for last disabled 
				if (!_shiftHolded || !_isList || !_isMultiple) {
					query("#" + childid + " ." + _styles.selected).removeClass(_styles.selected);
				};
	...
	var _previous = function () {
		...
		var selected = query("#" + childid + " li:visible." + _styles.selected);
		var items = query("#" + childid + " li:visible." + _styles_i.li);
		var index = query("#" + childid + " li:visible." + _styles_i.li).index(selected[0]);
		if (index >= 0) {
			index = getPrev(index);
			if (index >= 0) { //check again - hack for disabled 
				...
				if (parseInt(($(items[index]).position().top + $(items[index]).height())) <= 0) {
					var top = (query("#" + childid).scrollTop() - query("#" + childid).height()) - $(items[index]).height();
					query("#" + childid).animate({scrollTop: top}, 500);
				};
			};
			if (!_isList) {
	...
	var _adjustOpen = function () {
		var id = _getPostID("postID");
		var childid = _getPostID("postChildID");
		var pos = query("#" + id).offset();
		var mH = query("#" + id).height();
		var wH = $(window).height();
		var st = $(window).scrollTop();
		var cH = query("#" + childid).height();
		var top = query("#" + id).height(); //this close so its title height
		if ((wH + st) < Math.floor(cH + mH + pos.top) || _settings.openDirection.toLowerCase() == 'alwaysup') {
			top = cH;
			query("#" + childid).css({top: "-" + top + "px", display: 'block', zIndex: _settings.zIndex});
			query("#" + id).removeClass("borderRadius borderRadiusTp").addClass("borderRadiusBtm");
			var top = query("#" + childid).offset().top;
			if (top < -10) {
				query("#" + childid).css({top: (parseInt(query("#" + childid).css("top")) - top + 20 + st) + "px", zIndex: _settings.zIndex});
				query("#" + id).removeClass("borderRadiusBtm borderRadiusTp").addClass("borderRadius");
			};
		} else {
			query("#" + childid).css({top: top + "px", zIndex: _settings.zIndex});
			query("#" + id).removeClass("borderRadius borderRadiusBtm").addClass("borderRadiusTp");
		};
	};
	var _open = function (e) {
		...
		if (!_isOpen) {
			...
			if (msBeautify.oldDiv != '') {
				query("#" + msBeautify.oldDiv).css({display: "none"}); //hide all 
			};
			...
			if(animStyle=="" || animStyle=="none") {
				query("#" + childid).css({display:"block"});
				_scrollToIfNeeded();
				...
				query("#" + childid)[animStyle]("fast", function () {
					_scrollToIfNeeded();
					if (typeof _settings.on.open == "function") {
						var d = _getDataAndUI();
						_settings.on.open(d.data, d.ui);
	...
	var _close = function (e) {
		...
		if (_isList === false) {
			query("#" + childid).css({display: "none"});
			query("#" + id).removeClass("borderRadiusTp borderRadiusBtm").addClass("borderRadius");
			//do it onclick
			_fireAfterItemClicked();
		};
		_unbind_on_events();
	...
	var _getDataAndUIByOption = function (opt) {
		if (opt != null && typeof opt != "undefined") {
			...
			var ui = query("#" + childid + " li." + _styles_i.li + ":eq(" + (opt.index) + ")");
	...
	var _getDataAndUI = function () {
		...
		if (ele.selectedIndex == -1) {
			...
			ui = query("#" + childid + " li." + _styles.selected);
			if (ui.length > 1) {
				var d = [], op = [], ind = [];
	...
	var _updateTitleUI = function (index, byvalue) {
		...
		query("#" + titleid).find("." + _styles.label).html(value.text);
		getElement(titleid).className = _styles.ddTitleText + " " + value.className;
		//update desction		 
		if (value.description != "") {
			query("#" + titleid).find("." + _styles.description).html(value.description).show();
		} else {
			query("#" + titleid).find("." + _styles.description).html("").hide();
		};
		//update icon
		var img = query("#" + titleid).find("img");
		if (img.length > 0) {
			$(img).remove();
		};
		if (value.image != "" && _settings.showIcon) {
			img = _createElement("img", {src: value.image});
			query("#" + titleid).prepend(img);
			if(value.imagecss!="") {
		...
		switch (a) {
			case "add":
				...
				if (arguments.length == 3) {
					...
					index = query("#" + childid + " li." + _styles_i.li).length - 1;
				};				
				if (index < 0 || !index) {
					query("#" + childid + " ul").append(li);
				} else {
					var at = query("#" + childid + " li." + _styles_i.li)[index];
					$(at).before(li);
				};
				_removeChildEvents();
				_applyChildEvents();
				...
				break;
			case "remove":
				wasSelected = $(query("#" + childid + " li." + _styles_i.li)[i]).hasClass(_styles.selected);
				query("#" + childid + " li." + _styles_i.li + ":eq(" + i + ")").remove();
				var items = query("#" + childid + " li." + _styles.enabled);
				if (wasSelected == true) {
					if (items.length > 0) {
						$(items[0]).addClass(_styles.selected);
						var ind = query("#" + childid + " li." + _styles_i.li).index(items[0]);
						_setValue(ind);
					};
				};
				if (items.length == 0) {
				...
				if (query("#" + childid + " li." + _styles_i.li).length < _settings.visibleRows && !_isList) {					
					_childHeight(-1); //set autoheight
				};
	...
	this.set = function (prop, val) {
		...
		switch (prop) {
			...
			case "value":
				getElement(_element)[prop] = val;
				var childid = _getPostID("postChildID");
				query("#" + childid + " li." + _styles_i.li).removeClass(_styles.selected);
				$(query("#" + childid + " li." + _styles_i.li)[getElement(_element).selectedIndex]).addClass(_styles.selected);
			...
			case "length":
				...
				if (val < getElement(_element).length) {
					getElement(_element)[prop] = val;
					if (val == 0) {
						query("#" + childid + " li." + _styles_i.li).remove();
						_setValue(-1);
					} else {
						query("#" + childid + " li." + _styles_i.li + ":gt(" + (val - 1) + ")").remove();
						if (query("#" + childid + " li." + _styles.selected).length == 0) {
							query("#" + childid + " li." + _styles.enabled + ":eq(0)").addClass(_styles.selected);
						};
					};
					_updateProp(prop, val);
					_updateProp("children", getElement(_element)["children"]);
	...
	this.on = function (type, fn) {
		query("#" + _element).on(type, fn);
	};
	this.off = function (type, fn) {
		query("#" + _element).off(type, fn);
	};
	...
	this.destory = function () {
		...
		query("#" + id + ", #" + id + " *").off();
		query("#" + id).remove();
		query("#" + _element).parent().replaceWith(query("#" + _element));
		getElement(_element).onchange = function () { //restore
			if (_orginial.onchange != null) {
				_orginial.onchange.apply(this, arguments)
			};
		};
		query("#" + _element).data("dd", null);
	};
	//Create msDropDown		
	_construct();
};
