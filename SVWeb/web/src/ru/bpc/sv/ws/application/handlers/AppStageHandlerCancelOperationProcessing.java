...
public class AppStageHandlerCancelOperationProcessing extends AppStageHandler {

	private static Logger loggerDB = Logger.getLogger("PROCESSES_DB");
	private static final Logger logger = Logger.getLogger("SVAP");
	private static final String HANDLER_NAME = "AppStageHandlerCancelOperationProcessing";

	public void process() {
		long processBegin = System.currentTimeMillis();
		try {
			ApplicationsWsDao appWsDao = new ApplicationsWsDao();
			String result = appWsDao.cancelProcessing(getSessionId(), getApplicationId());
			...
			if (AppStageProcessor.trace) {
				logger.trace("APP: " + getApplicationId() + "; " + HANDLER_NAME + "; result = " +
						getStageResult());
				loggerDB.trace(new TraceLogInfo(getSessionId(), HANDLER_NAME + "; result = " +
						getStageResult(), EntityNames.APPLICATION, getApplicationId()));
			}
