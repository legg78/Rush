...
@ViewScoped
@ManagedBean (name = "MbFraudAlerts")
public class MbFraudAlerts extends AbstractBean {
	...
	private FraudAlert filter;

	private final DaoDataModel<FraudAlert> _alertsSource;
	private final TableRowSelection<FraudAlert> _itemSelection;
	private FraudAlert _activeFraudAlert;
	...
	private ArrayList<SelectItem> checks;
	...
	public MbFraudAlerts() {
		pageLink = "fraud|alerts";
		tabName = defaultTabName;
		thisBackLink = "fraud|alerts";
		
		_alertsSource = new DaoDataModel<FraudAlert>() {
			private static final long serialVersionUID = 5168175217200438246L;

			@Override
			protected FraudAlert[] loadDaoData(SelectionParams params) {
				if (!searching) {
					return new FraudAlert[0];
				}
				try {
					String tabNameParam = setFilters();
					if (tabNameParam != null) {
						return _fraudDao.getFraudAlertsProc(userSessionId, params,
								tabNameParam);
					}
				} catch (Exception e) {
					logger.error("", e);
			...
			protected int loadDaoDataSize(SelectionParams params) {
				...
				try {
					...
					if (tabNameParam != null) {
						return _fraudDao.getFraudAlertsCountProc(userSessionId, params,
								tabNameParam);
					}
				} catch (Exception e) {
					logger.error("", e);
		...
		_itemSelection = new TableRowSelection<FraudAlert>(null, _alertsSource);
	...
	private void restoreFilter(){
		HashMap<String,Object> queueFilter = getQueueFilter("MbFraudAlerts");
		if (queueFilter != null){
			
			if (queueFilter.containsKey("accountFilter")){
				filterAccount = (Account)queueFilter.get("accountFilter");
			}
			
			if (queueFilter.containsKey("operationFilter")){
				filterAuth = (Operation)queueFilter.get("operationFilter");
			}
			
			if (queueFilter.containsKey("cardFilter")){
				filterCard = (Card)queueFilter.get("cardFilter");
			}
			
			if (queueFilter.containsKey("terminalFilter")){
				filterTerminal = (Terminal)queueFilter.get("terminalFilter");
			}
			
			if (queueFilter.containsKey("entityType")){
				String entity = (String)queueFilter.get("entityType");
				if (entity.equalsIgnoreCase(EntityNames.AUTHORIZATION)){
					searchByAuthorization();
				}else if (entity.equalsIgnoreCase(EntityNames.ACCOUNT)){
					searchByAccount();
				}else if (entity.equalsIgnoreCase(EntityNames.CARD)){
					searchByCard();
				}else if (entity.equalsIgnoreCase(EntityNames.TERMINAL)){
					searchByTerminal();
				}
			}
		}
	...
	public DaoDataModel<FraudAlert> getFraudAlerts() {
		return _alertsSource;
	}

	public FraudAlert getActiveFraudAlert() {
		return _activeFraudAlert;
	}

	public void setActiveFraudAlert(FraudAlert activeFraudAlert) {
	...
	public void setFirstRowActive() {
		...
		SimpleSelection selection = new SimpleSelection();
		_activeFraudAlert = (FraudAlert) _alertsSource.getRowData();
		selection.addKey(_activeFraudAlert.getModelId());
		_itemSelection.setWrappedSelection(selection);
	...
	public void setFiltersAuth() {
		...
		if (filterAuth.getAuthId() != null){
			filters.add(new Filter("AUTH_ID", filterAuth.getAuthId()));
		}
		...
		if (checkId != null){
			filters.add(new Filter("CHECK_ID", checkId));
		}
	}
	...
	public FraudAlert getFilter() {
		if (filter == null) {
			filter = new FraudAlert();
		}
		return filter;
	}

	public void setFilter(FraudAlert filter) {
		this.filter = filter;
	}
	...
	public ArrayList<SelectItem> getChecks() {
		if (checks == null) {
			checks = (ArrayList<SelectItem>) getDictUtils().getLov(LovConstants.CHECK);
		}
		if (checks == null)
			checks = new ArrayList<SelectItem>();
		return checks;
	}
	...
	private void setFilterFormFraudAlert(Map<String, String> filterRec) throws ParseException {
		...
		if (filterRec.get("authId") != null){
			filterAuth.setAuthId(Long.parseLong(filterRec.get("authId")));
		}

		if (filterRec.get("currency") != null){
			filterAuth.setOperationCurrency(filterRec.get("currency"));
		}

		if (filterRec.get("amountTo") != null){
			setAuthAmountTo(new BigDecimal(filterRec.get("amountTo")));
		}

		if (filterRec.get("amountFrom") != null) {
			setAuthAmountFrom(new BigDecimal(filterRec.get("amountFrom")));
		}

		if (filterRec.get("dateFrom") != null) {
			setAuthDateFrom(df.parse(filterRec.get("dateFrom")));
		}

		if (filterRec.get("dateTo") != null) {
			setAuthDateTo(df.parse(filterRec.get("dateTo")));
		}
		...
		if (filterRec.get("checkId") != null ) {
			setCheckId(Integer.parseInt(filterRec.get("checkId")));
		}
	}
	...
	private void setFilterRecFraudAlert(Map<String, String> filterRec) {
		SimpleDateFormat df = new SimpleDateFormat(DatePatterns.DATE_PATTERN);
		if (filterAuth.getAuthId() != null) {
			filterRec.put("authId", filterAuth.getAuthId().toString());
		}
		if (filterAuth.getOperationCurrency() != null) {
			filterRec.put("currency", filterAuth.getOperationCurrency());
		}
		if (getAuthAmountTo() != null) {
			filterRec.put("amountTo", getAuthAmountTo().toString());
		}
		if (getAuthAmountFrom() != null) {
			filterRec.put("amountFrom", getAuthAmountFrom().toString());
		}
		if (getAuthDateFrom() != null) {
			filterRec.put("dateFrom", df.format(getAuthDateFrom()));
		}
		if (getAuthDateTo() != null) {
			filterRec.put("dateTo", df.format(getAuthDateTo()));
		}
		...
		if (getCheckId() != null ) {
			filterRec.put("checkId", getCheckId().toString());
		}
