create or replace package body cst_bof_ghp_api_dsp_init_pkg is
...
begin
    ...
    if nvl(i_value_null, com_api_const_pkg.FALSE) = com_api_const_pkg.FALSE then
        l_from  := l_from  || ', cst_bof_ghp_fin_msg f';
        l_where := l_where || ' and f.id = o.oper_id';
    end if;

    l_from  := l_from  || ', user_tab_columns c';
    l_where := l_where || ' and c.table_name = ''CST_BOF_GHP_FIN_MSG''';
...
begin
    ...
    l_from := l_from || ', user_tab_columns c';
    l_where := l_where || ' and c.table_name = ''CST_BOF_GHP_FEE''';
...
begin
    ...
    l_value :=
        case
            ...
            when l_trans_code in (cst_bof_ghp_api_const_pkg.TC_SALES_CHARGEBACK
...
begin
    ...
    if l_value_null = com_api_const_pkg.FALSE then
        l_from  := l_from  || ', cst_bof_ghp_card f';
        l_where := l_where || ' and f.id = o.oper_id';
    ...
    l_from  := l_from  || ', user_tab_columns c';
    l_where := l_where || ' and c.table_name = ''CST_BOF_GHP_CARD''';
...
begin
    ...
    if nvl(i_value_null, com_api_type_pkg.FALSE) = com_api_type_pkg.FALSE then
        l_from  := l_from  || ', cst_bof_ghp_fee f';
        l_where := l_where || ' and f.id = o.oper_id';
    end if;
...
begin
    ...
    l_from  := l_from  || ', rul_ui_mod_param_vw r';
    l_where := l_where || ' and r.name = ''GHP_DEBIT_CREDIT_INDICATOR''';
    l_where := l_where || ' and r.lang = l.lang';

    l_from  := l_from  || ', user_tab_columns c';
    l_where := l_where || ' and c.table_name  = ''CST_BOF_GHP_FRAUD''';
    ...
    make_cursor_statement(
      ...
      , i_from         => l_from
      , i_where        => l_where
      , i_system_name  => '''GHP_DEBIT_CREDIT_INDICATOR'''
      , i_mandatory    => nvl(i_mandatory, com_api_const_pkg.FALSE)
      , i_editable     => nvl(i_editable, com_api_const_pkg.TRUE)
      , i_data_type    => com_api_const_pkg.DATA_TYPE_CHAR
    );
...
begin
    ...
    l_from  := l_from  || ', rul_ui_mod_param_vw r';
    l_where := l_where || ' and r.name = ''GHP_TRANS_GENERATION_METHOD''';
    l_where := l_where || ' and r.lang = l.lang';
    ...
    make_cursor_statement(
      ...
      , i_from         => l_from
      , i_where        => l_where
      , i_system_name  => '''GHP_TRANS_GENERATION_METHOD'''
      , i_mandatory    => nvl(i_mandatory, com_api_const_pkg.FALSE)
      , i_editable     => nvl(i_editable, com_api_const_pkg.TRUE)
      , i_data_type    => com_api_const_pkg.DATA_TYPE_CHAR
    );
...
begin
    ...
    l_from  := l_from  || ', rul_ui_mod_param_vw r';
    l_where := l_where || ' and r.name = ''GHP_INSURANCE_YEAR''';
    l_where := l_where || ' and r.lang = l.lang';
    ...
    make_cursor_statement(
      ...
      , i_from         => l_from
      , i_where        => l_where
      , i_system_name  => '''GHP_INSURANCE_YEAR'''
      , i_mandatory    => com_api_const_pkg.TRUE
      , i_editable     => com_api_const_pkg.TRUE
      , i_data_type    => com_api_const_pkg.DATA_TYPE_CHAR
    );
...
begin
    ...
    l_from  := l_from  || ', rul_ui_mod_param_vw r';
    l_where := l_where || ' and r.name = ''GHP_ACCOUNT_SEQ_NUMBER''';
    l_where := l_where || ' and r.lang = l.lang';
    ...
    make_cursor_statement(
      ...
      , i_from         => l_from
      , i_where        => l_where
      , i_system_name  => '''GHP_ACCOUNT_SEQ_NUMBER'''
      , i_mandatory    => nvl(i_mandatory, com_api_const_pkg.TRUE)
      , i_editable     => nvl(i_editable, com_api_const_pkg.TRUE)
      , i_data_type    => com_api_const_pkg.DATA_TYPE_CHAR
    );
...
begin
    ...
    l_from  := l_from  || ', rul_ui_mod_param_vw r';
    l_where := l_where || ' and r.name = ''GHP_DOCUMENT_TYPE''';
    l_where := l_where || ' and r.lang = l.lang';

    l_from  := l_from  || ', user_tab_columns c';
    l_where := l_where || ' and c.table_name = ''CST_BOF_GHP_RETRIEVAL''';
    ...
    make_cursor_statement(
      ...
      , i_from         => l_from
      , i_where        => l_where
      , i_system_name  => '''GHP_DOCUMENT_TYPE'''
...
begin
    ...
    l_from  := l_from  || ', rul_ui_mod_param_vw r';
    l_where := l_where || ' and r.name = ''GHP_CARD_ISSUER_REF_NUM''';
    l_where := l_where || ' and r.lang = l.lang';
    ...
    make_cursor_statement(
      ...
      , i_from         => l_from
      , i_where        => l_where
      , i_system_name  => '''GHP_CARD_ISSUER_REF_NUM'''
      , i_char         => nvl('''' || i_value || '''', com_api_const_pkg.DATA_VARCHAR2_NULL_INIT)
      , i_mandatory    => nvl(i_mandatory, com_api_const_pkg.FALSE)
      , i_editable     => i_editable
      , i_data_type    => com_api_const_pkg.DATA_TYPE_CHAR
...
begin
    ...
    l_from  := l_from  || ', rul_ui_mod_param_vw r';
    l_where := l_where || ' and r.name = ''GHP_CANCELLATION_INDICATOR''';
    l_where := l_where || ' and r.lang = l.lang';
    ...
    make_cursor_statement(
      ...
      , i_from         => l_from
      , i_where        => l_where
      , i_system_name  => '''GHP_CANCELLATION_INDICATOR'''
      , i_char         => nvl('''' || i_value || '''', com_api_const_pkg.DATA_VARCHAR2_NULL_INIT)
      , i_mandatory    => nvl(i_mandatory, com_api_const_pkg.TRUE)
      , i_editable     => nvl(i_editable, com_api_const_pkg.TRUE)
      , i_data_type    => com_api_const_pkg.DATA_TYPE_CHAR
...
begin
    ...
    l_from  := l_from  || ', rul_ui_mod_param_vw r';
    l_where := l_where || ' and r.name = ''GHP_RESPONSE_TYPE''';
    l_where := l_where || ' and r.lang = l.lang';
    ...
    make_cursor_statement(
      ...
      , i_from         => l_from
      , i_where        => l_where
      , i_system_name  => '''GHP_RESPONSE_TYPE'''
...
begin
    ...
    dsp_api_shared_data_pkg.set_param(
      ...
      , i_value  => case l_trans_code
                        when cst_bof_ghp_api_const_pkg.TC_SALES   then cst_bof_ghp_api_const_pkg.TC_SALES_CHARGEBACK
                        when cst_bof_ghp_api_const_pkg.TC_VOUCHER then cst_bof_ghp_api_const_pkg.TC_VOUCHER_CHARGEBACK
                        when cst_bof_ghp_api_const_pkg.TC_CASH    then cst_bof_ghp_api_const_pkg.TC_CASH_CHARGEBACK
                    end
    );
...
begin
    dsp_api_shared_data_pkg.set_param(
        i_name   => 'TRANSACTION_CODE'
      , i_value  => cst_bof_ghp_api_const_pkg.TC_FEE_COLLECTION
    );
...
begin
    dsp_api_shared_data_pkg.set_param(
        i_name   => 'TRANSACTION_CODE'
      , i_value  => cst_bof_ghp_api_const_pkg.TC_FUNDS_DISBURSEMENT
    );
...
begin
    dsp_api_shared_data_pkg.set_param(
        i_name   => 'TRANSACTION_CODE'
      , i_value  => cst_bof_ghp_api_const_pkg.TC_FRAUD_ADVICE
    );
