CREATE OR REPLACE package body jcb_api_rule_proc_pkg is

procedure create_jcb_fin_message is
...
begin
    if opr_api_shared_data_pkg.g_auth.id is not null then
        ...
        if l_fin_id is not null then
            trc_log_pkg.debug(
                i_text          => 'Outgoing JCB message for operation [#1] already present with id [#2]'
            ...
            jcb_api_fin_pkg.create_from_auth (
