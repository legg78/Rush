CREATE OR REPLACE package body amx_api_rule_proc_pkg is

procedure create_amx_fin_message is
...
begin
    ...
    if opr_api_shared_data_pkg.g_auth.id is not null then
        ...
        if l_fin_id is not null then
            trc_log_pkg.debug(
                i_text          => 'Outgoing AMX message for operation [#1] already present with id [#2]'
            ...
            amx_api_fin_message_pkg.process_auth (
