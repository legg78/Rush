create or replace package body mcw_api_msg_pkg is
    ...
    ) return mcw_api_type_pkg.t_de003 is
    begin
        if i_de003_1 is not null then
            return i_de003_1 || nvl(i_de003_2, mcw_api_const_pkg.DEFAULT_DE003_2) || nvl(i_de003_3, mcw_api_const_pkg.DEFAULT_DE003_3);
        else
            return null;
        end if;
    end;
    ...
    ) return mcw_api_type_pkg.t_de022 is
    begin
        if (
    ...
    ) return mcw_api_type_pkg.t_de030 is
    begin
        if i_de030_1 is not null or i_de030_2 is not null then
            return (
                mcw_utl_pkg.pad_number(nvl(i_de030_1, 0), 12, 12)
                || mcw_utl_pkg.pad_number(nvl(i_de030_2, 0), 12, 12)
            );
        else
            return null;
        end if;
    ...
    ) return mcw_api_type_pkg.t_de043 is
        result            mcw_api_type_pkg.t_de043;
        formatted_de043_3 mcw_api_type_pkg.t_de043;
    begin
        if (
    ...
    begin
        if i_de043 is not null then
            ...
            if l_pos > 0 then
                ...
                l_pos_char := instr(i_de043, mcw_api_const_pkg.DE043_FIELD_DELIMITER, l_curr_pos) + 1;
                l_curr_pos := l_pos + 1;
            else
                com_api_error_pkg.raise_error(
                    i_error         => 'MCW_SUBFIELD_DELIMITER_NOT_FOUND'
                    , i_env_param1  => 'DE043'
                    , i_env_param2  => l_pos_char
            ...
            o_de043_4 := rtrim(substr(i_de043, l_pos_char, 10));
            o_de043_5 := rtrim(substr(i_de043, l_pos_char + 10, 3));
            o_de043_6 := rtrim(substr(i_de043, l_pos_char + 13, 3));
        end if;
    end;
    ...
    procedure pack_message (
        ...
        , i_de005           in mcw_api_type_pkg.t_de005
        , i_de006           in mcw_api_type_pkg.t_de006
        , i_de009           in mcw_api_type_pkg.t_de009
        , i_de010           in mcw_api_type_pkg.t_de010
        , i_de012           in mcw_api_type_pkg.t_de012
        , i_de014           in mcw_api_type_pkg.t_de014
        , i_de022           in mcw_api_type_pkg.t_de022
        , i_de022_1         in mcw_api_type_pkg.t_de022s
        , i_de022_2         in mcw_api_type_pkg.t_de022s
        , i_de022_3         in mcw_api_type_pkg.t_de022s
        ...
        , i_de022_11        in mcw_api_type_pkg.t_de022s
        , i_de022_12        in mcw_api_type_pkg.t_de022s
        , i_de023           in mcw_api_type_pkg.t_de023
        ...
        , i_de032           in mcw_api_type_pkg.t_de032
        , i_de033           in mcw_api_type_pkg.t_de033
        , i_de037           in mcw_api_type_pkg.t_de037
        , i_de038           in mcw_api_type_pkg.t_de038
        , i_de040           in mcw_api_type_pkg.t_de040
        , i_de041           in mcw_api_type_pkg.t_de041
        ...
        , i_de051           in mcw_api_type_pkg.t_de051
        , i_de054           in mcw_api_type_pkg.t_de054
        , i_de055           in mcw_api_type_pkg.t_de055
        ...
        , i_de095           in mcw_api_type_pkg.t_de095
        , i_de100           in mcw_api_type_pkg.t_de100
        , i_de111           in mcw_api_type_pkg.t_de111
        , i_de127           in mcw_api_type_pkg.t_de127
        , i_charset         in com_api_type_pkg.t_oracle_name
    ) is
        l_de048             mcw_api_type_pkg.t_de048;
        l_de062             mcw_api_type_pkg.t_de062;
        l_de123             mcw_api_type_pkg.t_de123;
        l_de124             mcw_api_type_pkg.t_de124;
        l_de125             mcw_api_type_pkg.t_de125;
        ...
        l_default_charset   com_api_type_pkg.t_oracle_name := mcw_api_const_pkg.g_default_charset;
        ...
        begin
            case
                when bit between 1   and 16  then l_bitmask_word1 := mcw_utl_pkg.bitor(l_bitmask_word1, power(2, 16 - bit));
                when bit between 17  and 32  then l_bitmask_word2 := mcw_utl_pkg.bitor(l_bitmask_word2, power(2, 32 - bit));
                when bit between 33  and 48  then l_bitmask_word3 := mcw_utl_pkg.bitor(l_bitmask_word3, power(2, 48 - bit));
                when bit between 49  and 64  then l_bitmask_word4 := mcw_utl_pkg.bitor(l_bitmask_word4, power(2, 64 - bit));
                when bit between 65  and 80  then l_bitmask_word5 := mcw_utl_pkg.bitor(l_bitmask_word5, power(2, 80 - bit));
                when bit between 81  and 96  then l_bitmask_word6 := mcw_utl_pkg.bitor(l_bitmask_word6, power(2, 96 - bit));
                when bit between 97  and 112 then l_bitmask_word7 := mcw_utl_pkg.bitor(l_bitmask_word7, power(2, 112 - bit));
                when bit between 113 and 128 then l_bitmask_word8 := mcw_utl_pkg.bitor(l_bitmask_word8, power(2, 128 - bit));
                else null;
            end case;
        end;
        ...
        procedure add_field (
            ...
            l_data              mcw_api_type_pkg.t_de_body;
        begin
    ...
    begin
        ...
        add_field(to_char(i_de012, mcw_api_const_pkg.DE012_DATE_FORMAT), 12);
        add_field(to_char(i_de014, mcw_api_const_pkg.DE014_DATE_FORMAT), 14);
        ...
        add_field(to_char(i_de073, mcw_api_const_pkg.DE073_DATE_FORMAT), 73);
        add_field(i_de093, 93);

        if i_mti = mcw_api_const_pkg.MSG_TYPE_ADMINISTRATIVE then
            add_field(i_de094, 94);
        else
            add_field(mcw_utl_pkg.pad_number(nvl(i_de094, 0), 11, 11), 94);
        end if;

        add_field(i_de095, 95);
        add_field(i_de100, 100);
        add_field(i_de111, 111);
        add_field(l_de123, 123);
        add_field(l_de124, 124);
        add_field(l_de125, 125);
        add_field(i_de127, 127);

        set_bit(1); -- bitmask2 always present;
        o_raw_data := utl_raw.cast_to_raw(convert_data(mcw_utl_pkg.pad_number(i_mti, 4, 4))) || pack_bitmask || o_raw_data;
    end;
    ...
    procedure unpack_message (
        ...
        begin
            ...
            if l_data is not null and ltrim(l_data, '0') is not null then
                begin
                    if i_fmt = mcw_api_const_pkg.DE073_DATE_FORMAT and substr(l_data, 5, 2) = '00' then
    ...
    begin
        ...
        get_field_date(o_de012, mcw_api_const_pkg.DE012_DATE_FORMAT, 12);
        get_field_date(o_de014, mcw_api_const_pkg.DE014_DATE_FORMAT, 14);
        ...
        get_field_date(o_de073, mcw_api_const_pkg.DE073_DATE_FORMAT, 73);
