create or replace package body itf_prc_import_pkg as
/*********************************************************
...
select
  ...
  , opr.acquirer_card_id
  , opr.acquirer_card_instance_id
  ...
  , opr.destination_card_instance_id
  ...
  , opr.aggregator_card_id
  , opr.aggregator_card_instance_id
       ...
       , xmltable(
           ...
           , '/operation'
             ...
             columns
               ...
               , issuer_prty_type                  varchar2(8)   path 'issuer/participant_type'
               ...
               , issuer_card_instance_id           number(12)    path 'issuer/card_instance_id'
               ...
               , acquirer_prty_type                varchar2(8)   path 'acquirer/participant_type'
               ...
               , acquirer_card_id                  number(12)    path 'acquirer/card_id'
               , acquirer_card_instance_id         number(12)    path 'acquirer/card_instance_id'
               ...
               , destination_prty_type             varchar2(8)   path 'destination/participant_type'
               ...
               , destination_card_instance_id      number(12)    path 'destination/card_instance_id'
               ...
               , aggregator_prty_type              varchar2(8)   path 'aggregator/participant_type'
               ...
               , aggregator_card_id                number(12)    path 'aggregator/card_id'
               , aggregator_card_instance_id       number(12)    path 'aggregator/card_instance_id'
               ...
               , srvp_prty_type                    varchar2(8)   path 'service_provider/participant_type'
               ...
               , srvp_card_id                      number(12)    path 'service_provider/card_id'
               , srvp_card_instance_id             number(12)    path 'service_provider/card_instance_id'
...
procedure register_events(
    io_oper              in out nocopy itf_prc_import_pkg.t_oper_clearing_rec
...
begin
    trc_log_pkg.debug(
        i_text          => 'itf_prc_import_pkg.register_auth_tag'
    );
...
begin
    trc_log_pkg.debug(
        i_text          => 'itf_prc_import_pkg.register_auth_data; i_oper_id [#1] start'
      , i_env_param1    => i_oper_id
    );
    ...
    trc_log_pkg.debug(
        i_text          => 'itf_prc_import_pkg.register_auth_data; i_oper_id [#1] end'
      , i_env_param1    => i_oper_id
    );
...
end register_additional_amount;
...
begin
    trc_log_pkg.debug(
        i_text          => 'itf_prc_import_pkg.register_custom_participants'
    );
...
end register_custom_participants;
...
begin
    ...
    then
        ...
        trc_log_pkg.warn(
          ...
          , i_env_param5 => i_merge_reversal
          , i_env_param6 => i_merge_sttl_type_id
        );
    end if;

    if i_merge_reversal = aut_api_const_pkg.MERGE_REVERSAL_NO_MERGE then
        null;

    elsif i_merge_reversal = aut_api_const_pkg.MERGE_REVERSAL_BY_AMOUNT then

        if io_oper.oper_amount_value = l_orig_amount and l_orig_oper_status = opr_api_const_pkg.OPERATION_STATUS_PROCESS_READY then
           io_status := opr_api_const_pkg.OPERATION_STATUS_MERGED;
           update opr_operation
              set status = io_status
            where id in (io_oper.oper_id, l_original_id);
        end if;

    elsif i_merge_reversal = aut_api_const_pkg.MERGE_REVERSAL_BY_AMOUNT_STTT then

        if io_oper.oper_amount_value = l_orig_amount
            and com_api_array_pkg.is_element_in_array(
                    i_array_id   => i_merge_sttl_type_id
                  , i_elem_value => io_oper.sttl_type
                ) = com_api_const_pkg.TRUE
            and l_orig_oper_status = opr_api_const_pkg.OPERATION_STATUS_PROCESS_READY then
           io_status := opr_api_const_pkg.OPERATION_STATUS_MERGED;
           update opr_operation
              set status = io_status
            where id in (io_oper.oper_id, l_original_id);
    ...
    for i in 1 .. g_operations.count loop
        ...
        exception
            when no_data_found then
                -- Error
                trc_log_pkg.error(
                    i_text        => 'itf_prc_import_pkg.update_total_amount: No found base operation. oper_id [#1]'
                  , i_env_param1  => g_operations(i)
                );
           when too_many_rows then
                -- Error
    ...
    begin
        ...
        then
            get_original_id (
              ...
              , o_original_id       => l_original_id
              , io_status           => l_status
              , i_merge_reversal     => i_merge_reversal
              , i_merge_sttl_type_id => i_merge_sttl_type_id
            );
            ...
            if nvl(io_oper.external_orig_id, 0) != nvl(io_oper.external_auth_id, 0) then
                if io_oper.is_reversal = com_api_const_pkg.FALSE then
                    select max(id)
                      into l_original_id
                      from aut_auth
                     where external_auth_id = io_oper.external_orig_id;
                else
                    select max(a.id)
                      into l_original_id
                      from aut_auth a
        ...
        if io_oper.issuer_exists = com_api_const_pkg.TRUE then
            ...
            l_issuer.card_id          := coalesce(
                                             io_oper.issuer_card_id
                                           , iss_api_card_pkg.get_card_id(i_card_number => io_oper.issuer_card_number)
                                         );
            l_issuer.card_instance_id := io_oper.issuer_card_instance_id;
        ...
        if io_oper.acquirer_exists = com_api_const_pkg.TRUE then
            ...
            l_acquirer.card_id          := io_oper.acquirer_card_id;
            l_acquirer.card_instance_id := io_oper.acquirer_card_instance_id;
        ...
        if io_oper.destination_exists = com_api_const_pkg.TRUE then
            ...
            l_dest.card_instance_id := io_oper.destination_card_instance_id;
        ...
        if io_oper.aggregator_exists = com_api_const_pkg.TRUE then
            ...
            l_agg.card_id          := io_oper.aggregator_card_id;
            l_agg.card_instance_id := io_oper.aggregator_card_instance_id;
        ...
        then
            if io_oper.service_provider_exists = com_api_const_pkg.TRUE then
                ...
                l_srvp.card_id          := io_oper.srvp_card_id;
                l_srvp.card_instance_id := io_oper.srvp_card_instance_id;
...
procedure register_operation_batch(
  ...
  , i_inst_id               in          com_api_type_pkg.t_tiny_id     default null
    ...
    l_oper_rec              itf_prc_import_pkg.t_oper_clearing_rec;
    ...
    l_merge_reversal        com_api_type_pkg.t_dict_value;
    l_merge_sttl_type_id    com_api_type_pkg.t_short_id;
begin
    ...
    l_merge_reversal     := set_ui_value_pkg.get_inst_param_v(
                                i_param_name => 'AUT_MERGE_REVERSAL'
                              , i_inst_id    => nvl(i_inst_id, ost_api_const_pkg.DEFAULT_INST)
                            );
    l_merge_sttl_type_id := set_ui_value_pkg.get_inst_param_n(
                                i_param_name => 'AUT_MERGE_STTL_TYPE_ID'
                              , i_inst_id    => nvl(i_inst_id, ost_api_const_pkg.DEFAULT_INST)
                            );
    ...
    if l_estimated_count > 0 then
        ...
        for i in 1 .. i_oper_tab.count loop
            ...
            l_oper_rec.original_id                      := i_oper_tab(i).original_id;
            ...
            l_oper_rec.issuer_prty_type                 := i_oper_tab(i).issuer_prty_type;
            ...
            l_oper_rec.issuer_card_instance_id          := i_oper_tab(i).issuer_card_instance_id;
            ...
            l_oper_rec.acquirer_prty_type               := i_oper_tab(i).acquirer_prty_type;
            ...
            l_oper_rec.acquirer_card_id                 := i_oper_tab(i).acquirer_card_id;
            l_oper_rec.acquirer_card_instance_id        := i_oper_tab(i).acquirer_card_instance_id;
            ...
            l_oper_rec.destination_prty_type            := i_oper_tab(i).destination_prty_type;
            ...
            l_oper_rec.destination_card_instance_id     := i_oper_tab(i).destination_card_instance_id;
            ...
            l_oper_rec.aggregator_prty_type             := i_oper_tab(i).aggregator_prty_type;
            ...
            l_oper_rec.aggregator_card_id               := i_oper_tab(i).aggregator_card_id;
            l_oper_rec.aggregator_card_instance_id      := i_oper_tab(i).aggregator_card_instance_id;
            ...
            l_oper_rec.srvp_prty_type                   := i_oper_tab(i).srvp_prty_type;
            ...
            l_oper_rec.srvp_card_id                     := i_oper_tab(i).srvp_card_id;
            l_oper_rec.srvp_card_instance_id            := i_oper_tab(i).srvp_card_instance_id;
            ...
            if i_auth_data_tab.count >= l_auth_data_index then
            ...
            if i_auth_tag_tab.count >= l_auth_tag_index then
            ...
            l_resp_code := register_operation(
                             ...
                             , i_without_checks     => i_without_checks
                             , i_merge_reversal     => l_merge_reversal
                             , i_merge_sttl_type_id => l_merge_sttl_type_id
