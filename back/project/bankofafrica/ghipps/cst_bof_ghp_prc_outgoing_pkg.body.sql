create or replace package body cst_bof_ghp_prc_outgoing_pkg as

BULK_LIMIT       constant integer  := 1000;

g_default_charset         com_api_type_pkg.t_oracle_name := cst_bof_ghp_api_const_pkg.g_default_charset;
g_charset                 com_api_type_pkg.t_oracle_name := cst_bof_ghp_api_const_pkg.g_default_charset;
...
function get_rec_header(
  ...
  , io_file                in out cst_bof_ghp_api_type_pkg.t_ghp_file_rec
...
procedure process_logical_file_header(
    io_file                in out  cst_bof_ghp_api_type_pkg.t_ghp_file_rec
  , o_logical_file            out  cst_bof_ghp_api_type_pkg.t_logical_file_rec
...
procedure process_logical_file_trailer(
    io_file                in out cst_bof_ghp_api_type_pkg.t_ghp_file_rec
  , io_logical_file        in out cst_bof_ghp_api_type_pkg.t_logical_file_rec
  , i_session_file_id      in     com_api_type_pkg.t_long_id
) is
    l_line                   com_api_type_pkg.t_text;
...
begin
    ...
    l_trans_code :=
       case io_logical_file.logical_file_code
           when cst_bof_ghp_api_const_pkg.TC_FM_HEADER  then cst_bof_ghp_api_const_pkg.TC_FM_TRAILER
           when cst_bof_ghp_api_const_pkg.TC_FV_HEADER  then cst_bof_ghp_api_const_pkg.TC_FV_TRAILER
           when cst_bof_ghp_api_const_pkg.TC_FMC_HEADER then cst_bof_ghp_api_const_pkg.TC_FMC_TRAILER
           when cst_bof_ghp_api_const_pkg.TC_FSW_HEADER then cst_bof_ghp_api_const_pkg.TC_FSW_TRAILER
           when cst_bof_ghp_api_const_pkg.TC_FL_HEADER  then cst_bof_ghp_api_const_pkg.TC_FL_TRAILER
       end;
    l_line :=
        get_rec_header(
...
procedure process_file_header(
  ...
  , i_session_file_id      in      com_api_type_pkg.t_long_id
  , o_file                    out  cst_bof_ghp_api_type_pkg.t_ghp_file_rec
) is
    l_line                         com_api_type_pkg.t_text;
    l_param_tab                    com_api_type_pkg.t_param_tab;
begin
    o_file.id              := cst_bof_ghp_file_seq.nextval;
    ...
    select nvl(max(to_number(ghp_file_id)), 0) + 1
      into o_file.ghp_file_id
    ...
    if o_file.originator_bin is null then
        com_api_error_pkg.raise_error(
              i_error       => 'GHP_ACQ_BUSINESS_ID_NOT_FOUND'
    ...
    l_line :=
        get_rec_header(
            i_trans_code => cst_bof_ghp_api_const_pkg.TC_FILE_HEADER
          , i_tcr        => '0'
          , io_file      => o_file
        );
    ...
    l_line := l_line || '001';                                -- File sequence number
    ...
    l_line := l_line || lpad(o_file.originator_bin, 6, '0');
...
begin
    l_line :=
        get_rec_header(
            i_trans_code => cst_bof_ghp_api_const_pkg.TC_FILE_TRAILER
          , i_tcr        => '0'
          , io_file      => io_file
        );
...
procedure process_draft(
    i_fin_message          in            cst_bof_ghp_api_type_pkg.t_ghp_fin_mes_rec
  , i_session_file_id      in            com_api_type_pkg.t_long_id
  , io_file                in out nocopy cst_bof_ghp_api_type_pkg.t_ghp_file_rec
) is
    l_line                               com_api_type_pkg.t_text;
...
begin
    ...
    l_line := l_line || case when l_oper_currency_exponent = 0                 -- source amount; pos 57
    ...
    l_line := l_line || rpad(i_fin_message.spec_chargeback_ind, 1, ' ');       -- special chargeback indicator; pos 77

    l_line := l_line || nvl(l_dest_currency_exponent, '0');                    -- destination currency exponent; pos 78
    l_line := l_line || case when l_dest_currency_exponent = 0                 -- destination amount; pos 79
                             then lpad(i_fin_message.dest_amount, 10, '0') || '00'
                             else lpad(i_fin_message.dest_amount, 12, '0')
                        end;
    ...
    l_line := l_line || case when l_dest_currency_exponent = 0
                             then lpad(i_fin_message.iss_reimb_fee, 10, '0') || '00'
                             else lpad(i_fin_message.iss_reimb_fee, 12, '0')
                        end;
    -- Value date; pos 115
...
begin
    ...
    cst_bof_ghp_api_fin_msg_pkg.get_fee(
...
procedure process_retrieval_request(
  ...
  , io_file                in out nocopy cst_bof_ghp_api_type_pkg.t_ghp_file_rec
    ...
    l_line                               com_api_type_pkg.t_text;
begin
    cst_bof_ghp_api_fin_msg_pkg.get_retrieval(
        i_id             => i_fin_message.id
...
procedure process_fraud_advice(
  ...
  , io_file                in out nocopy cst_bof_ghp_api_type_pkg.t_ghp_file_rec
    ...
    l_line                               com_api_type_pkg.t_text;
begin
    cst_bof_ghp_api_fin_msg_pkg.get_fraud(
        i_id         => i_fin_message.id
...
procedure process_logical_file(
    io_logical_file        in out nocopy cst_bof_ghp_api_type_pkg.t_logical_file_rec
  , i_trans_code           in            com_api_type_pkg.t_byte_char
  , i_usage_code           in            com_api_type_pkg.t_byte_char
) is
begin
    if i_trans_code = cst_bof_ghp_api_const_pkg.TC_SALES then
        io_logical_file.total_tc05 := io_logical_file.total_tc05 + 1;
    end if;

    if i_trans_code in (cst_bof_ghp_api_const_pkg.TC_SALES
    ...
    elsif i_trans_code in (cst_bof_ghp_api_const_pkg.TC_SALES_CHARGEBACK
    ...
    elsif i_trans_code in (cst_bof_ghp_api_const_pkg.TC_SALES_REVERSAL
    ...
    elsif i_trans_code in (cst_bof_ghp_api_const_pkg.TC_SALES_CHARGEBACK_REV
    ...
    elsif i_trans_code = cst_bof_ghp_api_const_pkg.TC_FEE_COLLECTION then
        if i_usage_code = '1' then
    ...
    elsif i_trans_code = cst_bof_ghp_api_const_pkg.TC_FUNDS_DISBURSEMENT then
        if i_usage_code = '1' then
    ...
    elsif i_trans_code = cst_bof_ghp_api_const_pkg.TC_REQUEST_ORIGINAL_PAPER then
        io_logical_file.total_tc51 := io_logical_file.total_tc51 + 1;

    elsif i_trans_code = cst_bof_ghp_api_const_pkg.TC_REQUEST_FOR_PHOTOCOPY then
        io_logical_file.total_tc52 := io_logical_file.total_tc52 + 1;

    elsif i_trans_code = cst_bof_ghp_api_const_pkg.TC_MAILING_CONFIRMATION then
        io_logical_file.total_tc53 := io_logical_file.total_tc53 + 1;

    elsif i_trans_code = cst_bof_ghp_api_const_pkg.TC_FRAUD_ADVICE then
...
procedure process(
  ...
  , i_charset              in      com_api_type_pkg.t_oracle_name  default null
    ...
    l_fin_message                  cst_bof_ghp_api_type_pkg.t_ghp_fin_mes_tab;

    l_session_file_id              com_api_type_pkg.t_long_id;

    l_file                         cst_bof_ghp_api_type_pkg.t_ghp_file_rec;
    l_logical_file                 cst_bof_ghp_api_type_pkg.t_logical_file_rec;
    l_trans_code                   varchar2(2);

    l_header_written               boolean := false;

    l_fin_cur                      cst_bof_ghp_api_type_pkg.t_ghp_fin_cur;
    ...
    begin
        ...
        prc_api_file_pkg.open_file(
          ...
          , i_file_type     => cst_bof_ghp_api_const_pkg.FILE_TYPE_CLEARING_GHP
          , io_params       => l_params
        );
...
begin
    trc_log_pkg.debug(
        i_text  => 'GHIPPS outgoing clearing START'
    );
    ...
    for i in 1..l_host_id.count loop
        l_record_count := cst_bof_ghp_api_fin_msg_pkg.estimate_messages_for_upload(
    ...
    if l_estimated_count > 0 then
        for i in 1..l_host_id.count loop
            ...
            cst_bof_ghp_api_fin_msg_pkg.enum_messages_for_upload(
                o_fin_cur       => l_fin_cur
              , i_network_id    => l_network_id(i)
              , i_inst_id       => l_inst_id(i)
              , i_host_inst_id  => l_host_inst_id(i)
            ...
            loop
                ...
                for j in 1..l_fin_message.count loop
                    ...
                    -- process fee collections and funds diburstment
                              ...
                              cst_bof_ghp_api_const_pkg.TC_FEE_COLLECTION
                    ...
                    -- process retrieval requests
                            ...
                            , cst_bof_ghp_api_const_pkg.TC_MAILING_CONFIRMATION
                          )
                    then
    ...
    trc_log_pkg.debug(
        i_text  => 'GHIPPS outgoing clearing END'
    );
