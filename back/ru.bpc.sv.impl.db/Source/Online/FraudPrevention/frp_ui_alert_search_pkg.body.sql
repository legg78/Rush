create or replace package body frp_ui_alert_search_pkg is
/********************************************************* 
  ...
  , i_is_first_call     in     com_api_type_pkg.t_boolean
    ...
    COLUMN_LIST         constant com_api_type_pkg.t_text :=
        ...
        'select f.id'||
             ...
             ', com_api_i18n_pkg.get_text(''frp_check'', ''label'', f.check_id, get_user_lang()) as check_name'||
             ', com_api_i18n_pkg.get_text(''frp_check'', ''description'', f.check_id, get_user_lang()) as check_desc'
             ;
    ...
    l_ref_source        com_api_type_pkg.t_text :=
        ...
        ||        ', :p_auth_id          as p_auth_id'
        ||        ', :p_check_id         as p_check_id'
        ||        ' from dual) x '
                ...
                when r.number_value is not null then
                    case
                        ...
                        when r.condition = '<=' then 
                            ...
                            ' and '||lower(i_param_name)||' '||nvl(r.condition, '=')||' p_'||lower(i_param_name)
                    end
            end;
        end loop;
    ...
    if i_is_first_call = com_api_const_pkg.TRUE then
        ...
        execute immediate l_ref_source 
          ...
          using l_amount_from
              ...
              , l_account_number --get_char_value('ACCOUNT_NUMBER')
              , l_terminal_number --get_char_value('TERMINAL_NUMBER')
              , get_number_value('AUTH_ID')
              , get_number_value('CHECK_ID')
