...
public class ApplicationsMigrateFileSaver extends AbstractFileSaver {
	...
	private final static String saverName = "ApplicationsMigrateFileSaver";
	...
	public void save() throws Exception {
		...
		logger.info(saverName + ": document parsed.");
		...
		if (applications instanceof Node
			...
			for (int s = 0; s < applications.getLength(); s++) {
				...
				if (ApplicationConstants.APPLICATION_MAIN_NODE.equalsIgnoreCase(application.getNodeName())) {
			...
			logger.info(saverName + ": " + THREAD_NUMBER + " arrays were prepared");
			for (int s = 0; s < THREAD_NUMBER; s++) {
	...
	private class ApplicationSaveTask implements Runnable {
		...
		public void run() {
			...
			try {
				...
				if (application != null) {
					...
					int threadCount = 0;
					for (Node appNode : applicationsList) {
						threadCount++;
						String xml = nodeToString(appNode);
		...
		private void setObjectIds(Long applicationId) throws Exception {
			String linkId = null;
...
//							String params = "appIdsMap = " + appIdsMap.toString(); 
//							logger.trace("THREAD: " + THREAD + "; APP: " + applicationId + "; " + saverName + "; createAppTree:" + params +";");
					...
					if (el.getFirstChild() != null || el.hasAttribute("value")) {
						...
						if (newEl.isComplex()) {
							...
							val = getFirstChildNodeValue(el, false);
						}
						if (isValue) {
							try {
								if (trace) {
								...
								if (newEl.isNumber()) {
									...
									Date date = javax.xml.bind.DatatypeConverter.parseDate(
											getFirstChildNodeValue(el, true)).getTime();
