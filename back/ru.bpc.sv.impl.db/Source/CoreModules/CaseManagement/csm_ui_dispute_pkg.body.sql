create or replace package body csm_ui_dispute_pkg as
/**************************************************
...
begin
    ...
    trc_log_pkg.debug(
        i_text       => 'Creating l_application = {flow_id [#1]; split_hash [#2]}; appl_status [#3]; reject_code [#4]}'
      , i_env_param1 => l_application.flow_id
      , i_env_param2 => l_application.split_hash
      , i_env_param3 => l_application.appl_status
      , i_env_param4 => l_application.reject_code
    );
    ...
    app_ui_application_pkg.add_application(
      ...
      , i_split_hash       => l_application.split_hash
    ...
    -- Add history
    csm_api_dispute_pkg.add_history(
        i_case_id         => l_application.id
...
begin
    ...
    l_application.flow_id :=
        case l_participant.participant_type
            when com_api_const_pkg.PARTICIPANT_ISSUER then
                case
                    when l_operation.merchant_country = l_participant.card_country
                    then app_api_const_pkg.FLOW_ID_ISS_DISPUTE_DOMESTIC
                    else app_api_const_pkg.FLOW_ID_ISS_DISPUTE_INTERNTNL
                end
            when com_api_const_pkg.PARTICIPANT_ACQUIRER then
                case
                    when l_operation.merchant_country = l_participant.card_country
                    then app_api_const_pkg.FLOW_ID_ACQ_DISPUTE_DOMESTIC
                    else app_api_const_pkg.FLOW_ID_ACQ_DISPUTE_INTERNTNL
                end
        end;
    ...
    if l_participant.customer_id is not null then
        l_application.split_hash := com_api_hash_pkg.get_split_hash(
...
begin
    ...
    csm_api_dispute_pkg.set_due_date(
        i_case_id       => i_case_id
      , i_due_date      => i_due_date
      , io_seqnum       => io_seqnum
    );
...
/
drop package csm_ui_dispute_pkg
/
