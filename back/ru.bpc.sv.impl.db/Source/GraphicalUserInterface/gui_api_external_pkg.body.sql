...
is
    ...
    l_cursor_tbl           com_api_type_pkg.t_text := 'from (select id as customer_id'
                                                   ...
                                                   ||            ', i.id_series||i.id_number as id_document '
                                                   ||         'from com_id_object i '
                                                   ||        'where i.id_number is not null '
                                                   ||          'and i.id_type is not null '
                                                   ||      ') i'
                                                   ||    ', com_person p'
...
is
    ...
    l_cursor_column        com_api_type_pkg.t_text := 'select /*+ ordered  use_nl(cu ic ai nct icn pc pp ci ch ich) */ '
                                                   ||      '  row_number() over(order by ic.id) as seq'
...
is
    ...
    l_cursor_column        com_api_type_pkg.t_text := 'select iv.invoice_id'
                                                   ...
                                                   ||        ') as overdue_balance'
                                                   ||      ', to_char('
                                                   ||            'pm.payment_amount / power(10, cc.exponent)'
                                                   ||          ', ''' || com_api_const_pkg.XML_NUMBER_FORMAT || ''''
                                                   ||            '|| rpad('
                                                   ||                   '''.'''
                                                   ||                 ', case cc.exponent '
    ...
    l_cursor_tbl           com_api_type_pkg.t_text := 'from prd_customer cu'
                                                   ...
                                                   ||         'from crd_invoice i'
                                                   ||      ') iv'
                                                   ||    ', (select sum(cp.amount) as payment_amount '
                                                   ...
                                                   ||          'and not exists (select 1 from opr_operation where original_id = oo.id and is_reversal = ' || com_api_const_pkg.TRUE || ') '
...
is
    ...
    LOG_PREFIX             constant com_api_type_pkg.t_name := lower($$PLSQL_UNIT) || '.get_crd_invoice_payments_info: ';
