create or replace package body cst_bmed_prc_incoming_pkg as
...
is
    LOG_PREFIX               constant com_api_type_pkg.t_name := lower($$PLSQL_UNIT) || '.process_import_account_status: ';
    l_record_count_all_files          com_api_type_pkg.t_long_id := 0;
    l_record_count                    com_api_type_pkg.t_long_id := 0;
    l_record_number                   com_api_type_pkg.t_long_id := 0;
    l_rec                             com_api_type_pkg.t_text;
    ...
    l_fe_acc_status                   com_api_type_pkg.t_byte_char;
    l_account_status                  com_api_type_pkg.t_dict_value;
    l_fe_acc_status_array_type_id     com_api_type_pkg.t_tiny_id  := nvl(i_fe_acc_status_array_type_id, cst_bmed_api_const_pkg.FE_ACC_STATUS_ARRAY_TYPE_ID);
    l_fe_acc_status_array_id          com_api_type_pkg.t_short_id := nvl(i_fe_acc_status_array_id, cst_bmed_api_const_pkg.FE_ACC_STATUS_ARRAY_ID);
...
begin
    ...
    loop
        ...
        begin
            savepoint sp_bmed_acc_status_file;
            ...
            loop
                l_record_number := r.record_number;
                l_rec := r.raw_data;
                l_account_number := trim(substr(l_rec, 1, 13));
                l_account := 
                    acc_api_account_pkg.get_account(
                        i_account_id     => null
                      , i_account_number => l_account_number
                    );
                l_fe_acc_status  := substr(l_rec, 44, 1);
                l_account_status := 
                    com_api_array_pkg.conv_array_elem_v(
                        i_array_type_id => l_fe_acc_status_array_type_id
                      , i_array_id      => l_fe_acc_status_array_id
                      , i_inst_id       => l_account.inst_id
                      , i_elem_value    => l_fe_acc_status
                ...
                evt_api_status_pkg.change_status(
                  ...
                  , i_reason            => cst_bmed_api_const_pkg.BAL_REF_FILE_UPLOAD_EVNT_TYPE
        ...
        exception
            when com_api_error_pkg.e_application_error then
                rollback to sp_bmed_acc_status_file;
...
end process_import_account_status;
