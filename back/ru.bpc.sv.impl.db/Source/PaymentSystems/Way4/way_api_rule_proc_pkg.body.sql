create or replace package body way_api_rule_proc_pkg is

procedure create_way_fin_message is
...
begin
    if opr_api_shared_data_pkg.g_auth.id is not null then
        ...
        if l_fin_id is not null then
            trc_log_pkg.debug(
                i_text          => 'Outgoing WAY message for operation [#1] already present with id [#2]'
            ...
            way_api_fin_message_pkg.process_auth(
...
end create_way_fin_message;

end way_api_rule_proc_pkg;
/
