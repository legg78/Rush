create or replace package body cst_bnv_rcn_processing_pkg is
...
procedure get_fin_message(
  ...
  , o_fin_rec         out cst_bnv_napas_api_type_pkg.t_bnv_napas_fin_mes_rec
...
procedure process_disputes(
  ...
  , i_end_date         in date
    ...
    l_fin_rec             cst_bnv_napas_api_type_pkg.t_bnv_napas_fin_mes_rec;
...
begin
    ...
    for r_fin_mes in (
        ...
        cst_bnv_napas_api_fin_msg_pkg.create_operation(
            i_fin_rec              => l_fin_rec
...
is
    ...
    function get_line(
        i_fin_rec                cst_bnv_napas_api_type_pkg.t_bnv_napas_fin_mes_rec
    ...
    begin
        ...
        l_line := l_line || '[F50]'  || lpad(nvl(i_fin_rec.sttl_currency   , cst_bnv_napas_api_const_pkg.BNV_CURR_CODE), 3);
    ...
    begin
        ...
        if i_oper_type = opr_api_const_pkg.OPER_TYPE_FT_TO_EXTERNAL_CREDI then
            ...
            if i_participant_type = cst_bnv_napas_api_const_pkg.PARTICIPANT_TYPE_ISS or l_card_inst_id is not null then
                l_session_file_id    := l_iss_session_file_id;
                l_iss_count          := l_iss_count + 1;
                if i_participant_type is null then
                    -- not exist in NAPAS
        ...
        if l_session_file_id is null then
            if i_participant_type = cst_bnv_napas_api_const_pkg.PARTICIPANT_TYPE_ACQ or i_acq_inst_id = l_inst_id then
...
begin
    ...
    select max(file_name) keep (dense_rank first order by f.file_date desc)
     ...
     where f.file_type = cst_bnv_napas_api_const_pkg.FILE_TYPE_RECON_INCOMING
       and f.id       >= l_start_id;
    ...
    open_file(
      ...
      , i_file_type      => cst_bnv_napas_api_const_pkg.FILE_TYPE_RECON_ISS
    );
    open_file(
        io_sess_file_id  => l_acq_session_file_id
      , i_file_type      => cst_bnv_napas_api_const_pkg.FILE_TYPE_RECON_ACQ
    );
    ...
    for r in (            
        ...
        , f as (
            select f.id
                 ...
                 , fl.participant_type
              from cst_bnv_napas_fin_msg f
        inner join cst_bnv_napas_card c      on (c.id      = f.id)
        inner join cst_bnv_napas_fin_file fl on (f.file_id = fl.id)
    ...
    loop
        ...
        if r.id is not null and r.oper_id is not null then
            if r.trans_amount = r.oper_amount and r.trans_currency = r.oper_currency then
                l_status     := cst_bnv_napas_api_const_pkg.MSG_STATUS_RECONCILED;
            else
                l_status     := cst_bnv_napas_api_const_pkg.MSG_STATUS_DIFFERENCE;
            end if;
        elsif r.id is not null then
            l_status         := cst_bnv_napas_api_const_pkg.MSG_STATUS_NOT_FOUND_IN_SV;
        end if;
        
        if r.id is not null then
            update cst_bnv_napas_fin_msg f
               set f.status        = l_status
                 , f.match_oper_id = r.oper_id
             where f.id = r.id;
        end if;
        
        if l_status is null or l_status != cst_bnv_napas_api_const_pkg.MSG_STATUS_RECONCILED then
            ...
            if r.oper_id is not null then
                ...
                l_fin_rec.terminal_number    := case when length(l_operation.terminal_number) >= 8
                                                   then substr(l_operation.terminal_number, -8)
                                                   else l_operation.terminal_number
                                                end;
                ...
                if l_fin_rec.trans_code is null then
                    l_fin_rec.trans_code := lpad(substr(net_api_map_pkg.get_network_type(
                                                          ...
                                                          , i_standard_id   => cst_bnv_napas_api_const_pkg.BNV_STANDARD_ID
            ...
            if l_status  = cst_bnv_napas_api_const_pkg.MSG_STATUS_DIFFERENCE then
