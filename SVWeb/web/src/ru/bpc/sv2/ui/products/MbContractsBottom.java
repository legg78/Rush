...
@ViewScoped
@ManagedBean(name = "MbContractsBottom")
public class MbContractsBottom extends AbstractBean {
		...
		_contractsSource = new DaoDataModel<Contract>() {
			...
			protected Contract[] loadDaoData(SelectionParams params) {
				...
				try {
					...
					params.setPrivilege(ProductPrivConstants.VIEW_CUSTOMER_CONTRACTS_TAB);
	...
	private void loadTab(String tab) {
		...
		if (tab.equalsIgnoreCase("customerTab")) {
			...
			MbCardsBottomSearch cardsSearch = (MbCardsBottomSearch) ManagedBeanWrapper
					.getManagedBean("MbCardsBottomSearch");
			cardsSearch.clearFilter();
			cardsSearch.getFilter().setCustomerId(_activeContract.getCustomerId());
			cardsSearch.search();
			...
			accsSearch.getFilter().setEntityType(_activeContract.getCustomerType());
			...
			MbTerminal terminalsBean = (MbTerminal) ManagedBeanWrapper.getManagedBean("MbTerminal");
			terminalsBean.clearFilter(false);
			terminalsBean.getFilter().setContractId(_activeContract.getId());
			terminalsBean.search();
