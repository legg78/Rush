...
is
begin
    -- Redefine event type (Credit life cycle) for charging interest using custom bunch types for overdue accounts
    return
        case
            when crd_invoice_pkg.get_last_invoice(
                     i_entity_type => acc_api_const_pkg.ENTITY_TYPE_ACCOUNT
                   , i_object_id   => i_account_id
                   , i_split_hash  => i_split_hash
                   , i_mask_error  => com_api_const_pkg.TRUE
                 ).aging_period > 0
            then cst_cfc_api_const_pkg.OVERDUE_INTEREST_CHARGE_EVENT
            else i_event_type
        end;
end;
