create or replace package body nps_prc_incoming_pkg as
...
exception
    when com_api_error_pkg.e_application_error or com_api_error_pkg.e_invalid_number then
        com_api_error_pkg.raise_error(
            i_error      => 'NAPAS_WRONG_AMOUNT_VALUE'
...
begin
    ...
    exception
        when com_api_error_pkg.e_application_error or com_api_error_pkg.e_invalid_number then
            com_api_error_pkg.raise_error(
                i_error      => 'NAPAS_WRONG_EXCHANGE_VALUE'
...
procedure assign_dispute(
    io_napas_fin_mes    in out nocopy nps_api_type_pkg.t_napas_fin_mes_rec
) is
    ...
    cursor match_cur is
        select min(m.id)            as id
          ...
          from nps_fin_message m
             , nps_card c
         where m.sys_trace_number     = io_napas_fin_mes.sys_trace_number
           and m.trans_date           = io_napas_fin_mes.trans_date
           and m.oper_amount          = io_napas_fin_mes.oper_amount
           and m.oper_currency        = io_napas_fin_mes.oper_currency
           and c.id                   = m.id
...
begin
    trc_log_pkg.debug (
      ...
      , i_env_param1  => io_napas_fin_mes.sys_trace_number
      , i_env_param2  => to_char(io_napas_fin_mes.trans_date, com_api_const_pkg.LOG_DATE_FORMAT)
      , i_env_param3  => io_napas_fin_mes.oper_amount
      , i_env_param4  => io_napas_fin_mes.oper_currency
      , i_env_param5  => iss_api_card_pkg.get_card_mask(io_napas_fin_mes.card_number)
    );

    l_card_number := iss_api_token_pkg.encode_card_number(
                         i_card_number => io_napas_fin_mes.card_number
                     );
    ...
    for rec in match_cur loop
        ...
        if rec.id is not null then
            ...
            io_napas_fin_mes.dispute_id  := rec.id;
    ...
    if io_napas_fin_mes.dispute_id is null then
        trc_log_pkg.warn (
            i_text           => 'NPS_ORIGINAL_OPERATION_IS_NOT_FOUND'
            , i_env_param1   => io_napas_fin_mes.id
            , i_env_param2   => io_napas_fin_mes.sys_trace_number
            , i_env_param3   => com_api_type_pkg.convert_to_char(io_napas_fin_mes.trans_date)
            , i_env_param4   => io_napas_fin_mes.oper_amount
            , i_env_param5   => io_napas_fin_mes.oper_currency
            , i_env_param6   => iss_api_card_pkg.get_card_mask(io_napas_fin_mes.card_number)
            , i_entity_type  => opr_api_const_pkg.ENTITY_TYPE_OPERATION
            , i_object_id    => io_napas_fin_mes.id
        );
    end if;

    -- assign a new dispute id
    if l_dispute_id is null then
        update nps_fin_message
           set dispute_id = io_napas_fin_mes.dispute_id
         where id         = io_napas_fin_mes.dispute_id;
...
procedure process_file_header(
  ...
  , o_napas_file               out nps_api_type_pkg.t_napas_file_rec
) is
...
begin
    ...
    o_napas_file.is_incoming     := com_api_const_pkg.TRUE;
    o_napas_file.network_id      := i_network_id;
    o_napas_file.proc_bin        := to_char(to_number(substr(i_header_data, instr(i_header_data, '[REV]') + 5, 8)));
    o_napas_file.proc_date       := to_date(substr(i_header_data, instr(i_header_data, '[DATE]') + 6, 8), 'DDMMYYYY');
    g_filedate                   := o_napas_file.proc_date;
    ...
    o_napas_file.inst_id :=
        get_inst_id_by_proc_bin(
            i_proc_bin       => o_napas_file.proc_bin
          , i_network_id     => i_network_id
          , i_standard_id    => i_standard_id
        );

    if o_napas_file.inst_id is null then
        com_api_error_pkg.raise_error(
            i_error       => 'NAPAS_BIN_NOT_REGISTERED'
          , i_env_param1  => o_napas_file.proc_bin
          , i_env_param2  => i_network_id
          , i_env_param3  => o_napas_file.inst_id
        );
    end if;

    o_napas_file.session_file_id  := i_session_file_id;
    o_napas_file.id               := nps_file_seq.nextval;
    o_napas_file.participant_type := i_file_participant_type;
...
procedure process_file_trailer(
    i_tc_buffer             in     nps_api_type_pkg.t_tc_buffer
  , io_napas_file           in out nps_api_type_pkg.t_napas_file_rec
) is
begin
    io_napas_file.total_records := get_value(i_tc_buffer(1), '[NOT]');

    insert into nps_file (
        id
      , is_incoming
      , is_returned
      , network_id
      ...
      , participant_type
        ...
        io_napas_file.id
      , io_napas_file.is_incoming
      , io_napas_file.is_returned
      , io_napas_file.network_id
      , io_napas_file.proc_bin
      , io_napas_file.proc_date
      , io_napas_file.inst_id
      , io_napas_file.session_file_id
      , io_napas_file.total_records
      , io_napas_file.participant_type
    );
end;
...
procedure process_draft(
  ...
  , i_is_dispute            in     com_api_type_pkg.t_boolean
    ...
    l_napas_fin_mes                nps_api_type_pkg.t_napas_fin_mes_rec;
...
begin
    ...
    -- Message fields
    l_napas_fin_mes.id                 := opr_api_create_pkg.get_id;
    l_napas_fin_mes.mti                := get_value(l_tc_buffer, '[MTI]');
    l_napas_fin_mes.card_number        := trim(get_value(l_tc_buffer, '[F2]'));
    l_napas_fin_mes.trans_code         := trim(get_value(l_tc_buffer, '[F3]'));
    l_napas_fin_mes.file_id            := i_file_id;
    l_napas_fin_mes.record_number      := i_record_number;
    l_napas_fin_mes.service_code       := trim(get_value(l_tc_buffer, '[SVC]'));
    l_napas_fin_mes.channel_code       := trim(get_value(l_tc_buffer, '[TCC]'));
    l_napas_fin_mes.oper_currency      := trim(get_value(l_tc_buffer, '[F49]'));

    l_napas_fin_mes.oper_amount        := prepare_amount(
                                              i_amount_str     => get_value(l_tc_buffer, '[F4]')
                                            , i_curr_code      => l_napas_fin_mes.oper_currency
                                            , i_exponent       => 2
                                            , i_amount_desc    => 'Transaction amount'
                                          );

    l_napas_fin_mes.real_amount        := prepare_amount(
    ...
    l_napas_fin_mes.sttl_currency      := trim(get_value(l_tc_buffer, '[F50]'));
    -- Check settlement currency if it not equal '000' then take settlement amount else oper amount
    if l_napas_fin_mes.sttl_currency != '000' then
        l_napas_fin_mes.sttl_amount    := prepare_amount(
                                              i_amount_str     => get_value(l_tc_buffer, '[F5]')
                                            , i_curr_code      => l_napas_fin_mes.sttl_currency
                                            , i_exponent       => 2
                                            , i_amount_desc    => 'Settlement amount'
                                          );
    else
        l_napas_fin_mes.sttl_currency  := l_napas_fin_mes.oper_currency;
        l_napas_fin_mes.sttl_amount    := l_napas_fin_mes.oper_amount;
    end if;

    l_napas_fin_mes.bill_currency      := trim(get_value(l_tc_buffer, '[F51]'));
    l_napas_fin_mes.bill_amount        := prepare_amount(
                                              i_amount_str     => get_value(l_tc_buffer, '[F6]')
                                            , i_curr_code      => l_napas_fin_mes.bill_currency
                                            , i_exponent       => 2
                                            , i_amount_desc    => 'Billing amount'
                                          );
    l_napas_fin_mes.bill_real_amount   := prepare_amount(
    ...
    l_napas_fin_mes.bill_exchange_rate := prepare_exchange_rate(
    ...
    l_napas_fin_mes.sys_trace_number   := to_number(get_value(l_tc_buffer, '[F11]'));
    l_napas_fin_mes.trans_date         := date_mmddhhmiss(
                                              get_value(l_tc_buffer, '[F13]') 
                                           || get_value(l_tc_buffer, '[F12]') 
                                          );

    l_napas_fin_mes.sttl_date          := date_mmdd(        get_value(l_tc_buffer, '[F15]' ));
    l_napas_fin_mes.mcc                := trim(             get_value(l_tc_buffer, '[F18]' ));
    l_napas_fin_mes.pos_entry_mode     := to_number(        get_value(l_tc_buffer, '[F22]' ));
    l_napas_fin_mes.pos_condition_code := to_number(        get_value(l_tc_buffer, '[F25]' ));
    l_napas_fin_mes.terminal_number    := trim(             get_value(l_tc_buffer, '[F41]' ));
    l_napas_fin_mes.acq_inst_bin       := to_char(to_number(get_value(l_tc_buffer, '[ACQ]' )));
    l_napas_fin_mes.iss_inst_bin       := to_char(to_number(get_value(l_tc_buffer, '[ISS]' )));
    l_napas_fin_mes.merchant_number    := trim(             get_value(l_tc_buffer, '[MID]' ));
    l_napas_fin_mes.bnb_inst_bin       := to_char(to_number(get_value(l_tc_buffer, '[BNB]' )));
    l_napas_fin_mes.src_account_number := trim(             get_value(l_tc_buffer, '[F102]'));
    l_napas_fin_mes.dst_account_number := trim(             get_value(l_tc_buffer, '[F103]'));

    l_napas_fin_mes.iss_fee_napas      := prepare_amount(
    ...
    l_napas_fin_mes.iss_fee_acq        := prepare_amount(
    ...
    l_napas_fin_mes.iss_fee_bnb        := prepare_amount(
    ...
    l_napas_fin_mes.acq_fee_napas      := prepare_amount(
    ...
    l_napas_fin_mes.acq_fee_iss        := prepare_amount(
    ...
    l_napas_fin_mes.acq_fee_bnb        := prepare_amount(
    ...
    l_napas_fin_mes.bnb_fee_napas      := prepare_amount(
    ...
    l_napas_fin_mes.bnb_fee_iss        := prepare_amount(
    ...
    l_napas_fin_mes.bnb_fee_acq        := prepare_amount(
    ...
    l_napas_fin_mes.rrn                := trim(     get_value(l_tc_buffer, '[F37]'));
    l_napas_fin_mes.auth_code          := trim(     get_value(l_tc_buffer, '[F38]'));
    l_napas_fin_mes.transaction_id     := trim(     get_value(l_tc_buffer, '[TRN]'));
    l_napas_fin_mes.resp_code          := to_number(get_value(l_tc_buffer, '[RRC]'));

    if l_napas_fin_mes.oper_amount  != l_napas_fin_mes.real_amount and l_napas_fin_mes.oper_amount != 0 then
        l_napas_fin_mes.is_reversal    := com_api_const_pkg.TRUE;
    else
        l_napas_fin_mes.is_reversal    := com_api_const_pkg.FALSE;
    end if;

    l_napas_fin_mes.inst_id            := i_inst_id;
    l_napas_fin_mes.network_id         := i_network_id;
    l_napas_fin_mes.is_dispute         := i_is_dispute;
    l_napas_fin_mes.status             := nps_api_const_pkg.MSG_STATUS_LOADED;

    if l_napas_fin_mes.is_dispute    = com_api_const_pkg.TRUE then
        assign_dispute(
            io_napas_fin_mes => l_napas_fin_mes
        );
    end if;

    l_napas_fin_mes.id := nps_api_fin_message_pkg.put_message(
                              i_fin_rec => l_napas_fin_mes
                          );
    
    trc_log_pkg.debug(LOG_PREFIX || 'END');
    trc_log_pkg.debug(
        i_text       => LOG_PREFIX || '>> l_napas_fin_mes.id [#1]'
      , i_env_param1 => l_napas_fin_mes.id
    );
...
procedure process(
    ...
    l_tc_buffer                    nps_api_type_pkg.t_tc_buffer;
    l_napas_file                   nps_api_type_pkg.t_napas_file_rec;
...
begin
    ...
    for p in (
        ...
        begin
            savepoint sp_napas_incoming_file;
            ...
            for r in (
                ...
                if l_napas_file.id is null and l_tc != nps_api_const_pkg.TC_FILE_HEADER then
                    com_api_error_pkg.raise_error(
                        i_error       => 'NAPAS_FILE_MISSING_HEADER'
                      , i_env_param1  => l_napas_file.id
                    );
                end if;
                ...
                if l_first_tc = nps_api_const_pkg.TC_FILE_HEADER then
                    process_file_header(
                      ...
                      , o_napas_file            => l_napas_file
                    );

                -- process currency convertional rate updates
                elsif l_first_tc = nps_api_const_pkg.TC_FILE_TRAILER then
                    process_file_trailer(
                        i_tc_buffer         => l_tc_buffer
                      , io_napas_file       => l_napas_file
                    );
                    l_trailer_found  := com_api_const_pkg.TRUE;

                -- process draft transactions
                elsif l_first_tc = nps_api_const_pkg.TC_DRAFT then
                    process_draft(
                        i_tc_buffer            => l_tc_buffer
                      , i_network_id           => i_network_id
                      , i_inst_id              => l_napas_file.inst_id
                      , i_proc_date            => l_napas_file.proc_date
                      , i_file_id              => l_napas_file.id
                      , i_record_number        => l_record_number
                      , i_proc_bin             => l_napas_file.proc_bin
                ...
                if r.rn_desc = 1 then
                    ...
                    if l_trailer_found = com_api_const_pkg.FALSE then
                        com_api_error_pkg.raise_error(
                            i_error       => 'NAPAS_FILE_MISSING_TRAILER'
                          , i_env_param1  => l_napas_file.id
                        );
                    end if;
                end if;
        ...
        exception
            when com_api_error_pkg.e_application_error then
                rollback to sp_napas_incoming_file;
