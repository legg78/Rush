...
@ViewScoped
@ManagedBean (name = "MbUserLoginsWidget")
public class MbUserLoginsWidget implements Serializable {
	private static final long serialVersionUID = 1L;

	private static final Logger logger = Logger.getLogger("DASHBOARD");
	private static final String path = "/pages/widgets/user_sessions.jspx";
	...
	private HashMap<Integer, UserLoginsWidget> widgets;
	...
	public void init() {
		...
		widgets = new HashMap<Integer, UserLoginsWidget>(dashboard2widgetList.length);
		for (Dashboard2WidgetItem item : dashboard2widgetList) {
			for (WidgetItem widget : widgetList) {
				...
				if (path.equals(widget.getPath()) && widget.getId().equals(item.getWidgetId())) {
					widgets.put(item.getId(), new UserLoginsWidget(item.getId(), widget));
				}
			}
		}
	}

	public HashMap<Integer, UserLoginsWidget> getWidgets() {
		return widgets;
	}
	...
	public class UserLoginsWidget {
		...
		public ProcessUserSession[] getSessionsList() {
			if (sessionsList == null) {
				...
				try {
					sessionsList = _processDao.getProcessUserSessions(userSessionId, params);
				} catch (Exception e) {
					logger.error("", e);
					FacesUtils.addMessageError(e);
					sessionsList = new ProcessUserSession[0];
				}
			}
			return sessionsList;
		}
		...
		public void setFilters() {
			...
			sdf.setTimeZone(TimeZone.getTimeZone(commonUtils.getTimeZoneId()));
			paramFilter = new Filter();
			paramFilter.setElement("startDate");
			paramFilter.setValue(sdf.format(new Date()));
			filters.add(paramFilter);
			
			paramFilter = new Filter();
			paramFilter.setElement("ipAddressNot");
			paramFilter.setValue("OLTP");
			filters.add(paramFilter);
