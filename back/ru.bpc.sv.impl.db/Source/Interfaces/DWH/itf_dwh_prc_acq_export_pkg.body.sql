create or replace package body itf_dwh_prc_acq_export_pkg is

procedure process_merchant_common(
  ...
  , i_ver                 in     com_api_type_pkg.t_attr_name
) is
    DEFAULT_PROCEDURE_NAME  constant com_api_type_pkg.t_name  := 'ITF_DWH_PRC_ACQ_EXPORT_PKG.PROCESS_MERCHANT';
    LOG_PREFIX    constant com_api_type_pkg.t_name := lower($$PLSQL_UNIT) || '.process_merchant_common: ';
    ...
    -- version switchs
    l_show_merch_card       com_api_type_pkg.t_boolean        := com_api_const_pkg.FALSE;
    -- end of switchs
    ...
    begin
        if l_object_id_tab.count > 0 then
            trc_log_pkg.debug(
                i_text => 'Creating a new XML file...'
            );
            l_session_file_id := 0;
            ...
            trc_log_pkg.debug(
                i_text       => 'Estimated count of records is [#1]'
            ...
            trc_log_pkg.debug(
                i_text => 'file saved, count=' || l_counter || ', length=' || length(l_file)
            );
    ...
    exception
        when others then
            trc_log_pkg.debug(
                'Error while generate_xml ' || sqlerrm
            );
            if l_session_file_id != 0 then
                prc_api_file_pkg.close_file(
                    i_sess_file_id  => l_session_file_id
    ...
    begin
        trc_log_pkg.debug(
            i_text => 'Opening a cursor for all reconrs those are processed...'
        );
        ...
        trc_log_pkg.debug(
            i_text => 'Cursor was opened...'
        );
    end;

begin
    trc_log_pkg.debug(
        i_text       => LOG_PREFIX || '[#1]: START with l_full_export [#2], i_inst_id [#3], i_count [#4]'
      , i_env_param1 => i_ver
    ...
    -- fill switchs depends on ver
    if i_ver = '1.2' then
        l_show_merch_card := com_api_const_pkg.TRUE;
    end if;
    ...
    loop
        begin
            savepoint sp_dwh_merchant_export;
            ...
            if l_full_export = com_api_const_pkg.TRUE then
                ...
                trc_log_pkg.debug(
                    i_text => 'Records were fetched from cursor cur_objects [' || cur_objects%rowcount || ']'
                );
        ...
        exception
            when others then
                rollback to sp_dwh_merchant_export;
                raise;
        end;
    end loop;
    close cur_objects;
...
procedure process_merchant_1_3(
    i_inst_id             in     com_api_type_pkg.t_inst_id
  , i_full_export         in     com_api_type_pkg.t_boolean       default null
  , i_lang                in     com_api_type_pkg.t_dict_value    default null
  , i_count               in     com_api_type_pkg.t_medium_id     default null
...
procedure process_terminal_1_0(
    i_inst_id             in     com_api_type_pkg.t_inst_id
  , i_full_export         in     com_api_type_pkg.t_boolean       default null
  , i_lang                in     com_api_type_pkg.t_dict_value    default null
  , i_count               in     com_api_type_pkg.t_medium_id     default null
) is
    DEFAULT_PROCEDURE_NAME  constant com_api_type_pkg.t_name  := 'ITF_DWH_PRC_ACQ_EXPORT_PKG.PROCESS_TERMINAL';
              ...
              , xmlagg(
                    xmlelement(
                      ...
                      , xmlforest(
                          ...
                          , t.pos_batch_support                 as "pos_batch_support"
                        )
                      , (
...
begin
    trc_log_pkg.debug(
        i_text       => 'process_terminal_1_0: START with l_full_export [#1], i_inst_id [#2], i_count [#3]'
    ...
    loop
        begin
            savepoint sp_dwh_terminal_export;
        ...
        exception
            when others then
                rollback to sp_dwh_terminal_export;
                raise;
        end;
    end loop;
    close cur_objects;
    ...
    trc_log_pkg.debug('process_terminal_1_0: FINISH');
exception
    when others then
        prc_api_stat_pkg.log_end(
            i_result_code   => prc_api_const_pkg.PROCESS_RESULT_FAILED
...
procedure process_terminal_1_1(
    i_inst_id             in     com_api_type_pkg.t_inst_id
  , i_full_export         in     com_api_type_pkg.t_boolean       default null
  , i_lang                in     com_api_type_pkg.t_dict_value    default null
  , i_count               in     com_api_type_pkg.t_medium_id     default null
...
begin
    trc_log_pkg.debug(
        i_text       => 'process_terminal_1_1: START with l_full_export [#1], i_inst_id [#2], i_count [#3]'
      , i_env_param1 => l_full_export
      , i_env_param2 => i_inst_id
      , i_env_param3 => i_count
    );
    ...
    trc_log_pkg.debug('process_terminal_1_1: FINISH');
exception
    when others then
        prc_api_stat_pkg.log_end(
            i_result_code   => prc_api_const_pkg.PROCESS_RESULT_FAILED
...
begin
    ...
    if i_dwh_version = '1.0' then
        process_terminal_1_0(
            i_inst_id     => i_inst_id
          , i_full_export => i_full_export
          , i_lang        => i_lang
          , i_count       => i_count
        );
    elsif i_dwh_version between '1.1' and '1.2' then
        process_terminal_1_1(
            i_inst_id     => i_inst_id
          , i_full_export => i_full_export
          , i_lang        => i_lang
          , i_count       => i_count
