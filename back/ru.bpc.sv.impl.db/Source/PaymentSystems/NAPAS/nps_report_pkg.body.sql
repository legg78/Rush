create or replace package body nps_api_report_pkg as
...
begin
    trc_log_pkg.debug(
        i_text        => 'nps_api_report_pkg.reconciliate_results_not_napas [#1][#2][#3]'
    ...
    select xmlelement("header",
             ...
             , xmlelement("start_date", to_char(i_start_date, nps_api_const_pkg.XML_DATETIME_FORMAT))
             , xmlelement("end_date", to_char(i_end_date, nps_api_const_pkg.XML_DATETIME_FORMAT))
     ...
     select xmlelement("details"
             , xmlagg(
                   xmlelement("detail"
                     ...
                     , xmlelement("transaction_date", to_char(o.oper_date, nps_api_const_pkg.XML_DATETIME_FORMAT))
      ...
      where o.id between l_start_id and l_end_id
        ...
        and not exists (select 1 from nps_fin_message m
    ...
    trc_log_pkg.debug ( i_text => 'nps_api_report_pkg.reconciliate_results_not_napas Finished' );
...
begin
    trc_log_pkg.debug(
        i_text        => 'nps_api_report_pkg.reconciliate_results_not_sv [#1][#2][#3]'
      , i_env_param1  => i_start_date
      , i_env_param2  => i_end_date
      , i_env_param3  => i_lang
    );
    ...
    select xmlelement("header",
     ...
     select xmlelement("details"
             , xmlagg(
                   xmlelement("detail"
                     ...
                     , xmlelement("status", decode(m.status
                                                 , nps_api_const_pkg.MSG_STATUS_NOT_FOUND_IN_SV
      ...
      where m.id = c.id
        ...
        and m.status in (nps_api_const_pkg.MSG_STATUS_NOT_FOUND_IN_SV
                       , nps_api_const_pkg.MSG_STATUS_DIFFERENCE)
      order by m.status;
    ...
    trc_log_pkg.debug ( i_text => 'nps_api_report_pkg.reconciliate_results_not_sv Finished' );
