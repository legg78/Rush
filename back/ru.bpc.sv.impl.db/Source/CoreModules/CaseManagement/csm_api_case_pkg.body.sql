create or replace package body csm_api_case_pkg as
/*********************************************************
    ...
    then
        begin
            select get_flow_id(
                 ...
                 , p.auth_code
                 , o.acq_inst_bin
                 , nvl(p.card_network_id, p.network_id)
                 , o.id
...
begin
    l_operation_rec := opr_api_operation_pkg.get_operation(i_operation_id);
    return get_flow_id(l_operation_rec.sttl_type);
end get_flow_id;
...
begin    
    trc_log_pkg.debug(
        i_text       => 'get_flow_id: sttt [#1]'
      , i_env_param1 => i_sttl_type
    );
    
    l_flow_id :=
    case
       when com_api_array_pkg.is_element_in_array(
                i_array_id     => csm_api_const_pkg.CSM_STTL_INTERNAL_ARRAY
              , i_elem_value   => i_sttl_type
            ) = com_api_const_pkg.TRUE
       then app_api_const_pkg.FLOW_ID_DISPUTE_INTERNAL

       when com_api_array_pkg.is_element_in_array(
                i_array_id     => csm_api_const_pkg.CSM_STTL_ISS_INTERNATION_ARRAY
              , i_elem_value   => i_sttl_type
            ) = com_api_const_pkg.TRUE
       then app_api_const_pkg.FLOW_ID_ISS_DISPUTE_INTERNTNL
       
       when com_api_array_pkg.is_element_in_array(
                i_array_id     => csm_api_const_pkg.CSM_STTL_ISS_DOMESTIC_ARRAY
              , i_elem_value   => i_sttl_type
            ) = com_api_const_pkg.TRUE
       then app_api_const_pkg.FLOW_ID_ISS_DISPUTE_DOMESTIC
       
       when com_api_array_pkg.is_element_in_array(
                i_array_id     => csm_api_const_pkg.CSM_STTL_ACQ_INTERNATION_ARRAY
              , i_elem_value   => i_sttl_type
            ) = com_api_const_pkg.TRUE
       then app_api_const_pkg.FLOW_ID_ACQ_DISPUTE_INTERNTNL
       
       when com_api_array_pkg.is_element_in_array(
                i_array_id     => csm_api_const_pkg.CSM_STTL_ACQ_DOMESTIC_ARRAY
              , i_elem_value   => i_sttl_type
            ) = com_api_const_pkg.TRUE
       then app_api_const_pkg.FLOW_ID_ACQ_DISPUTE_DOMESTIC
       
       else null
    end;
    
    trc_log_pkg.debug(
        i_text       => 'get_flow_id: flow_id [#1]'
...
is
    ...
    l_old_reject_code    com_api_type_pkg.t_dict_value;
begin
    trc_log_pkg.debug('csm_api_case_pkg.check_due_date: i_case_id=' || i_case_id
    ...
    trc_log_pkg.debug('csm_api_case_pkg.check_due_date: l_oper_id=' || l_oper_id
    ...
    trc_log_pkg.debug('csm_api_case_pkg.check_due_date: l_due_date=' || l_due_date);
...
is
    ...
    l_reversal               com_api_type_pkg.t_boolean;
begin
    trc_log_pkg.debug(
    ...
    trc_log_pkg.debug(
        i_text       => 'network [#1], case_progress [#2]'
      , i_env_param1 => l_network_id
      , i_env_param2 => l_case_progress
    );
    ...
    trc_log_pkg.debug(
        i_text       => '(old)appl_status [#1], (old)reject_code [#2], flow [#3]'
      , i_env_param1 => l_old_appl_status
      , i_env_param2 => l_old_reject_code
      , i_env_param3 => l_flow_id
    );

    if l_flow_id in (app_api_const_pkg.FLOW_ID_ACQ_DISPUTE_DOMESTIC, app_api_const_pkg.FLOW_ID_ISS_DISPUTE_DOMESTIC) then
        l_standard_id := net_api_network_pkg.get_offline_standard(
            i_network_id => l_network_id
        );
        
        csm_api_progress_pkg.get_msg_type(
            i_network_id    => l_network_id
          , i_case_progress => l_case_progress
          , i_mask_error    => com_api_const_pkg.TRUE
          , o_msg_type      => l_message_type
          , o_is_reversal   => l_reversal
        );
        
        l_is_incoming := 
        csm_api_progress_pkg.get_is_incoming(
            i_flow_id     => l_flow_id
          , i_msg_type    => l_message_type
          , i_is_reversal => l_reversal
        );
        ...
        trc_log_pkg.debug(
            i_text       => LOG_PREFIX || ' setting due date to [#1]'
          , i_env_param1 => l_due_date 
        );
        set_due_date(
            i_case_id  => i_case_id
