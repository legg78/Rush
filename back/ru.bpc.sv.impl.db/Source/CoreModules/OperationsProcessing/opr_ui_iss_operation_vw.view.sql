create or replace force view opr_ui_iss_operation_vw as
select 'ENTTOPER' entity_type
     , o.id
     , o.status
     , decode(o.status_reason, 'AUSR0101', t.resp_code, o.status_reason) status_reason
     , o.sttl_type
     , o.msg_type
     , o.oper_type
     , o.oper_reason
 ...
 where a.oper_id(+) = o.id
   ...
   and t.id(+) = o.id
   and (i.inst_id in (select inst_id from acm_cu_inst_vw)
        or 
        d.inst_id in (select inst_id from acm_cu_inst_vw)
       )
/