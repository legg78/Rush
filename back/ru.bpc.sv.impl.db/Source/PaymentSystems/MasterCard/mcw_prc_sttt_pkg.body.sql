create or replace package body mcw_prc_sttt_pkg is
    ...
    begin
        ...
        union
            select min(id)
                 ...
                 , min(p0394_1)
                 , sum(p0394_2)
                 , mcw_api_const_pkg.CREDIT as p0395_1
                 , sum(nvl( 
                      case p0395_1
                          -- credit to destination
                          when mcw_api_const_pkg.CREDIT then
                              p0395_2
                          -- debit to destination
                          when mcw_api_const_pkg.DEBIT then
                              -p0395_2
                          else
                              0
                      end
             ...
             where decode(status, 'CLMS0040', network_id, null) = i_network_id
               and de025 = '6862'
             group by de050, de025, de049 , de093, p0165, p0372_1, p0372_2, p0374, p0378, file_id   
             ;                     
    end;
    ...
    procedure open_fpd_fin_cursor(
      ...
      , io_original_file_id_tab  in out com_api_type_pkg.t_number_tab
        ...
        l_fpd_file_p105             mcw_api_type_pkg.t_p0105;
    begin
        if i_fpd_rec.de025 = mcw_api_const_pkg.FPD_REASON_NOTIFICATION then
            l_is_incoming := com_api_type_pkg.TRUE;
        elsif i_fpd_rec.de025 = mcw_api_const_pkg.FPD_REASON_ACKNOWLEDGEMENT then
            l_is_incoming := com_api_type_pkg.FALSE;
        end if;

        if i_fpd_rec.de049 is null
           and not (i_fpd_rec.mti = mcw_api_const_pkg.MSG_TYPE_ADMINISTRATIVE 
                    and i_fpd_rec.de024 = mcw_api_const_pkg.FUNC_CODE_FPD) then
            com_api_error_pkg.raise_error(
                i_error         => 'MCW_MANDATORY_RECONCIL_CATEGORY_EMPTY'
                , i_env_param1  => 'DE049'
            );
        ...
        elsif i_fpd_rec.p0302 is not null then
            com_api_error_pkg.raise_error(
                i_error         => 'MCW_UNSUPPORTED_RECONCIL_CATEGORY'
                , i_env_param1  => 'P0302'
                , i_env_param2  => i_fpd_rec.p0302
            );
        elsif i_fpd_rec.p0370_1 is not null then
            com_api_error_pkg.raise_error(
                i_error         => 'MCW_UNSUPPORTED_RECONCIL_CATEGORY'
                , i_env_param1  => 'P0370_1'
                , i_env_param2  => i_fpd_rec.p0370_1
            );
        elsif i_fpd_rec.p0370_2 is not null then
            com_api_error_pkg.raise_error(
                i_error         => 'MCW_UNSUPPORTED_RECONCIL_CATEGORY'
                , i_env_param1  => 'P0370_2'
                , i_env_param2  => i_fpd_rec.p0370_2
            );
        elsif (i_fpd_rec.p0358_1  is not null or
               i_fpd_rec.p0358_2  is not null or
               i_fpd_rec.p0358_3  is not null or
               i_fpd_rec.p0358_4  is not null or
               i_fpd_rec.p0358_5  is not null or
               i_fpd_rec.p0358_6  is not null or
               i_fpd_rec.p0358_7  is not null or
               i_fpd_rec.p0358_8  is not null or
               i_fpd_rec.p0358_9  is not null or
               i_fpd_rec.p0358_10 is not null or
               i_fpd_rec.p0358_11 is not null or
               i_fpd_rec.p0358_12 is not null or
               i_fpd_rec.p0358_13 is not null or
               i_fpd_rec.p0358_14 is not null)
        then
        ...
        exception when others then
            com_api_error_pkg.raise_error(
                i_error         => 'MCW_CANNOT_FIND_ORIGINAL_FILE'
        ...
        begin
            ...
            select f.id
             ...
             where f.p0105       = decode(i_fpd_rec.de025
                                        , mcw_api_const_pkg.FPD_REASON_NOTIFICATION,    decode(substr(l_fpd_file_p105, 21, 2), '99', l_fpd_file_p105, i_fpd_rec.p0300)
                                        , mcw_api_const_pkg.FPD_REASON_ACKNOWLEDGEMENT, i_fpd_rec.p0300
                                   )
        ...
        exception
            ...
            when too_many_rows then
                com_api_error_pkg.raise_error(
                    i_error         => 'MCW_TOO_MANY_ORIGINAL_FILES'
                    , i_env_param1  => i_fpd_rec.p0300
                );
        end;
        ...
        if i_fpd_rec.p0375 is not null then
            ...
            open o_cursor for
                select f.rowid, f.id, f.impact, f.is_incoming, f.mti
                     , (
                ...
                where f.p0375               = i_fpd_rec.p0375
                  and f.file_id+0           = l_file_id         -- disable index for the "file_id, network_id" fields
                  and f.network_id          = i_network_id
                  and f.is_incoming         = l_is_incoming
                  and f.is_rejected         = com_api_type_pkg.FALSE
                  and f.is_fpd_matched      = com_api_type_pkg.FALSE
                  and decode(l_is_incoming, com_api_type_pkg.TRUE, f.de093, f.de094) = i_fpd_rec.de093
                  ...
                  and (i_fpd_rec.p0374   is null or f.de003_1 = i_fpd_rec.p0374)
                  and (i_fpd_rec.de049   is null or f.de049   = i_fpd_rec.de049)
                  and (nvl(i_fpd_rec.p0378, DUMMY_CHAR) != mcw_api_const_pkg.REVERSAL_PDS_ORIGINAL
                       or nvl(f.p0025_1, mcw_api_const_pkg.REVERSAL_PDS_CANCEL) = mcw_api_const_pkg.REVERSAL_PDS_CANCEL)
                  and (nvl(i_fpd_rec.p0378, DUMMY_CHAR) != mcw_api_const_pkg.REVERSAL_PDS_REVERSAL
                       or f.p0025_1    = mcw_api_const_pkg.REVERSAL_PDS_REVERSAL)
                  and (nvl(i_fpd_rec.de025, DUMMY_CHAR) != mcw_api_const_pkg.FPD_REASON_NOTIFICATION
    ...
    procedure build_fsum_fin_where (
        ...
        l_fsum_file_p105           mcw_api_type_pkg.t_p0105;        
    begin
        l_where_stmt := l_fin_tab_alias || 'is_rejected=' || com_api_type_pkg.FALSE;

        if i_fsum_rec.de025 = mcw_api_const_pkg.FPD_REASON_NOTIFICATION then
            l_is_incoming := com_api_type_pkg.TRUE;
            l_where_stmt := l_where_stmt || ' and ' || l_fin_tab_alias || 'de093=' || i_fsum_rec.de093;
        elsif i_fsum_rec.de025 = mcw_api_const_pkg.FPD_REASON_ACKNOWLEDGEMENT then
            l_is_incoming := com_api_type_pkg.FALSE;
        ...
        begin
            ...
            from
                mcw_file f
            where -- for fsum with several cycle files use p0105 from fsum file
                f.p0105 = decode(i_fsum_rec.de025, '6862', decode(substr(l_fsum_file_p105, 21, 2), '99', l_fsum_file_p105, i_fsum_rec.p0300), '6861', i_fsum_rec.p0300)
        ...
        l_where_stmt := l_where_stmt || ' and ' || '(('|| i_fsum_rec.local_file ||' = 1 and ' || l_fin_tab_alias || 'local_message = 1 and ' || l_fin_tab_alias || 'p0165 != ''C'')';
        l_where_stmt := l_where_stmt || ' or ' || '('|| i_fsum_rec.local_file ||' = 0 and (' || l_fin_tab_alias || 'local_message = 0 or ' || l_fin_tab_alias || 'p0165 = ''C'')))';
    
        o_statement := l_where_stmt;
    end;

    procedure init_total (
        io_total_rec            out nocopy mcw_api_type_pkg.t_reconcile_total_rec
    ) is
    begin
    ...
    procedure process_summary (
        ...
        l_fsum_tab                  mcw_api_type_pkg.t_fsum_tab;
        
        l_debits                    mcw_api_type_pkg.t_reconcile_total_rec;
        l_credits                   mcw_api_type_pkg.t_reconcile_total_rec;
        ...
        l_amount_mandatory          com_api_type_pkg.t_boolean;
        ...
        begin
            l_sel_stmt := 
                'select ' 
                  ||    SEL_FLD_STMT 
                  || ', p.id fpd_id'
                  || ', nvl(decode(p.p0395_1, '''||mcw_api_const_pkg.CREDIT||''', p.p0395_2,'''||mcw_api_const_pkg.DEBIT||''', -p.p0395_2, 0), 0) amt_fee'
                  || ', nvl(p.p0394_2,0) amt_trn'
                  || ' from mcw_fin f, mcw_fpd p, mcw_card c';
        ...
        procedure register_skip_grouped (
            i_id                    in com_api_type_pkg.t_long_id
        ) is
        begin
            for rec in(
            ...
            loop
                l_skip_id(l_skip_id.count + 1) := rec.id;
            end loop;        
        end;
    ...
    begin
        ...
        savepoint starting_mcw_file_summary;
        
        l_amount_mandatory := set_ui_value_pkg.get_system_param_n('RECONCILIATION_MANDATORY_AMOUNT');
        ...
        loop
            ...
            for i in 1 .. l_fsum_tab.count loop
                ...
                trc_log_pkg.debug (
                        i_text       => 'Summary DEBUG [#1]: i=' || i || ', p0400=' || l_fsum_tab(i).p0400 || ', p0401=' || l_fsum_tab(i).p0401
                                        || ', p0380_1=' || l_fsum_tab(i).p0380_1 || ', p0381_1=' || l_fsum_tab(i).p0381_1 || ', de025=' || l_fsum_tab(i).de025
                      , i_env_param1 => l_msg_prefix 
                );
                begin
                    savepoint processing_next_fsum;
                            
                    if l_fsum_tab(i).de025 in (mcw_api_const_pkg.FPD_REASON_ACKNOWLEDGEMENT, mcw_api_const_pkg.FPD_REASON_NOTIFICATION) then
                        null;
                    else
                        com_api_error_pkg.raise_error(
                    ...
                    if l_fsum_tab(i).p0300 like '9%' then
                        trc_log_pkg.debug (
                        ...
                        if l_fsum_tab(i).is_grouped = com_api_type_pkg.TRUE then     
                            register_skip_grouped (
                                i_id     => l_fsum_tab(i).id
                            );
                        end if;    
                        ...
                        (   (   l_fsum_tab(i).p0400 = l_debits.count_transaction and
                            ...
                            com_api_error_pkg.raise_error(
                                i_error         => 'MCW_TOTALS_COUNT_NOT_EQUAL'
                            );
                        end if;

                    if (
                           l_fsum_tab(i).p0380_2 = l_debits.amount_transaction and l_fsum_tab(i).p0381_2 = l_credits.amount_transaction
                        or l_fsum_tab(i).p0380_1 is null and l_fsum_tab(i).p0381_1 is null and l_amount_mandatory = com_api_const_pkg.FALSE
                       ) then
                            null;
                        else
                            com_api_error_pkg.raise_error(
                                i_error         => 'MCW_TOTALS_NOT_EQUAL'
                            );
                        end if;
                        ...
                        if l_fsum_tab(i).de025 = mcw_api_const_pkg.FPD_REASON_ACKNOWLEDGEMENT then
        ...
        trc_log_pkg.debug (
            ...
            , i_env_param5 => l_excepted_count || chr(10)
            , i_env_param6 => l_skiped_count
        );
        
    exception
        when others then
            rollback to savepoint starting_mcw_file_summary;
            if l_fin_cur%isopen then
                close l_fin_cur;
            end if;
    ...
    procedure process_unsettled(
        io_original_file_id_tab  in out com_api_type_pkg.t_number_tab
      , i_eff_date               in     date
    ) is
    begin
        if io_original_file_id_tab.count > 0 then
            for i in 1..io_original_file_id_tab.count
            loop
                for rec in (select f.id
                                 , f.inst_id
                                 , f.split_hash
                              from mcw_fin f
                             where f.file_id  = io_original_file_id_tab(i)
                               and f.fpd_id  is null)
                loop
                    evt_api_event_pkg.register_event(
                        i_event_type   => opr_api_const_pkg.EVENT_SETTLEMENT_UNSUCCESSFUL
    ...
    procedure process_settlement (
        ...
        l_fpd_tab                   mcw_api_type_pkg.t_fpd_tab;
        ...
        begin  
            ...
            select/*+ ordered use_nl(t,f2) use_hash(f1) index(f2 MCW_FPD_PK) index(f1 MCW_FPD_CLMS0040_NDX) */ f1.id 
        ...
        procedure register_skip_settled (
            i_id                    in com_api_type_pkg.t_long_id   
        ) is
        begin
            l_skip_id(l_skip_id.count + 1) := i_id;
        ...
        procedure mark_skip_settled is
        begin
            forall i in 1 .. l_skip_id.count
                update mcw_fpd
                   set status = net_api_const_pkg.CLEARING_MSG_STATUS_MATCH_SKIP
        ...
        procedure check_skip_settled is
        begin
            if l_skip_id.count >= BULK_LIMIT then
                mark_skip_settled;
            end if;
        ...
        procedure event_register(
            i_event_type        in com_api_type_pkg.t_dict_value
          , i_eff_date          in date
          , i_object_id         in com_api_type_pkg.t_number_tab
          , i_inst_id           in com_api_type_pkg.t_number_tab
          , i_split_hash        in com_api_type_pkg.t_number_tab
        ) is
        begin
            for i in 1..i_object_id.count
            loop
                evt_api_event_pkg.register_event(
                    i_event_type   => i_event_type
                  , i_eff_date     => i_eff_date
                  , i_entity_type  => opr_api_const_pkg.ENTITY_TYPE_OPERATION
                  , i_object_id    => i_object_id(i)
    ...
    begin
        ...
        savepoint starting_mcw_settlement;
        ...
        --l_reconciliation_mode := mcw_api_const_pkg.RECONCILIATION_MODE_NONE;
        --l_reconciliation_mode := mcw_api_const_pkg.RECONCILIATION_MODE_FULL;
        ...
        if l_reconciliation_mode = mcw_api_const_pkg.RECONCILIATION_MODE_NONE then
            ...
            update mcw_fpd d
               set d.status = decode (
                                  d.de025
                                , mcw_api_const_pkg.FPD_REASON_ACKNOWLEDGEMENT, net_api_const_pkg.CLEARING_MSG_STATUS_MATCH_SKIP
                                , mcw_api_const_pkg.FPD_REASON_NOTIFICATION, net_api_const_pkg.CLEARING_MSG_STATUS_MATCH_SKIP
                                , net_api_const_pkg.CLEARING_MSG_STATUS_MATCH_ERR
                              )
             where decode(d.status, 'CLMS0040', d.network_id, null) = i_network_id
             returning count(*)
                     , count( case when d.de025 = mcw_api_const_pkg.FPD_REASON_ACKNOWLEDGEMENT then 1 end )
                     , count( case when d.de025 = mcw_api_const_pkg.FPD_REASON_NOTIFICATION then 1 end )
                     , count( case when d.de025 not in (mcw_api_const_pkg.FPD_REASON_ACKNOWLEDGEMENT, mcw_api_const_pkg.FPD_REASON_NOTIFICATION) then 1 end )
                  into l_total_count
                     , l_outgoing_count
                     , l_incoming_count
                     , l_excepted_count
            ...
            loop
                ...
                for i in 1 .. l_fpd_tab.count loop
                    ...
                    trc_log_pkg.debug (
                            i_text       => 'Reconciliation DEBUG [#1]: i=' || i || ', p0400=' || l_fpd_tab(i).p0400 || ', p0401=' || l_fpd_tab(i).p0401
                                            || ', p0380_1=' || l_fpd_tab(i).p0380_1 || ', p0381_1=' || l_fpd_tab(i).p0381_1 || ', de025=' || l_fpd_tab(i).de025
                          , i_env_param1 => l_msg_prefix 
                    );
                    ...
                    begin
                        ...
                        if l_fpd_tab(i).p0300 like '9%' then
                            trc_log_pkg.debug (
                                i_text       => '[#1] Reconciliation skipped, P0300 [#2]'
                              , i_env_param1 => l_msg_prefix
                              , i_env_param2 => l_fpd_tab(i).p0300
                            );

                            register_skip_settled (
                                i_id         => l_fpd_tab(i).id
                            );

                        else
                        
                            if l_fpd_tab(i).de025 in (mcw_api_const_pkg.FPD_REASON_ACKNOWLEDGEMENT, mcw_api_const_pkg.FPD_REASON_NOTIFICATION) then
                                null;
                            else
                                com_api_error_pkg.raise_error(
                                    i_error           => 'MCW_UNKNOWN_REASON'
                            ...
                            loop
                                ...
                                for k in 1 .. l_rowid.count loop
                                    ...
                                    if l_impact(k) = com_api_type_pkg.CREDIT then
                                        l_credits.amount_transaction    := l_credits.amount_transaction
                                                                           + nvl(case
                                                                                     when l_fpd_tab(i).de025 in (mcw_api_const_pkg.FPD_REASON_ACKNOWLEDGEMENT)
                                                                                     then l_de004(k)
                                                                                     else l_de005(k)
                                                                                 end, 0);
                                        ...
                                        l_debits.amount_transaction    := l_debits.amount_transaction
                                                                          + nvl(case
                                                                                    when l_fpd_tab(i).de025 in (mcw_api_const_pkg.FPD_REASON_ACKNOWLEDGEMENT)
                                                                                    then l_de004(k)
                                                                                    else l_de005(k)
                            ...
                            if l_fpd_tab(i).de025 in (mcw_api_const_pkg.FPD_REASON_ACKNOWLEDGEMENT) then 
                                trc_log_pkg.debug (
                                ...
                                trc_log_pkg.debug (
                                  ...
                                  , i_env_param1 => 'fpd.p0400=' || l_fpd_tab(i).p0400   || ' debits.count='  || l_debits.count_transaction  || ' debits.count_vs_fee='  || l_debits.count_vs_fee  || chr(10)
                                  , i_env_param2 => 'fpd.p0401=' || l_fpd_tab(i).p0401   || ' credits.count=' || l_credits.count_transaction || ' credits.count_vs_fee=' || l_credits.count_vs_fee || chr(10)
                                  , i_env_param3 => 'fpd.p0390=' || l_fpd_tab(i).p0390_2 || ' debits.amount_transaction='  || l_debits.amount_transaction || chr(10)
                                  , i_env_param4 => 'fpd.p0391=' || l_fpd_tab(i).p0391_2 || ' credits.amount_transaction=' || l_credits.amount_transaction
                                );
                            end if;
                            ...
                            if (
                                   l_fpd_tab(i).p0380_2      = l_debits.amount_transaction
                                   and l_fpd_tab(i).p0381_2  = l_credits.amount_transaction 
                                   and l_fpd_tab(i).de025   in (mcw_api_const_pkg.FPD_REASON_ACKNOWLEDGEMENT)
                                or l_fpd_tab(i).p0380_1     is null 
                                   and l_fpd_tab(i).p0381_1 is null 
                                   and l_amount_mandatory    = com_api_const_pkg.FALSE
                               )
                               or (
                                   l_fpd_tab(i).p0390_2      = l_debits.amount_transaction
                                   and l_fpd_tab(i).p0391_2  = l_credits.amount_transaction 
                                   and l_fpd_tab(i).de025   in (mcw_api_const_pkg.FPD_REASON_NOTIFICATION)
                                or l_fpd_tab(i).p0380_1     is null 
                                   and l_fpd_tab(i).p0381_1 is null 
                                   and l_amount_mandatory    = com_api_const_pkg.FALSE
                               )
                            ...
                            event_register(
                                i_event_type    => opr_api_const_pkg.EVENT_SETTLEMENT_SUCCESSFUL
                            ...
                            if l_fpd_tab(i).is_grouped = com_api_type_pkg.TRUE then
                                l_ok_grouped_id.extend;
                                l_ok_grouped_id(l_ok_grouped_id.count) := l_fpd_tab(i).id;
                            end if;

                            if l_fpd_tab(i).de025 = mcw_api_const_pkg.FPD_REASON_ACKNOWLEDGEMENT then
                                l_outgoing_count := l_outgoing_count + 1;
                            elsif l_fpd_tab(i).de025 = mcw_api_const_pkg.FPD_REASON_NOTIFICATION then
                                l_incoming_count := l_incoming_count + 1;
                            end if;
                        end if;
    ...
    exception
        when others then
            rollback to savepoint starting_mcw_settlement;
            if l_fin_cur%isopen then
                close l_fin_cur;
            end if;
