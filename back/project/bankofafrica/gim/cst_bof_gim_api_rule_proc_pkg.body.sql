create or replace package body cst_bof_gim_api_rule_proc_pkg is
...
begin
    if opr_api_shared_data_pkg.g_auth.id is not null then
        ...
        if l_fin_id is not null then
            trc_log_pkg.debug(
                i_text          => 'Outgoing GIM-UEMOA message for operation [#1] already present with id [#2]'
            ...
            cst_bof_gim_api_fin_msg_pkg.process_auth(
...
procedure load_gim_message_params
is
...
begin
    ...
    select min(network_id)
      into l_iss_network_id
    ...
    opr_api_shared_data_pkg.set_param(
      ...
      , i_value     => l_iss_network_id
    );
end load_gim_message_params;
